...
describe('every route resolves to a section', () => {
  ...
  it('maps every section id in the registry to a route', () => {
    ...
    /**
     * 24, not 20. The four additions are `/reference` — the **only reading surface** in the
     * application — `/token-exchange`, which the server implemented and Module 06 taught while the
     * debugger could not send it, `/native-sso`, a direct probe of the server's dedicated
     * `/api/nativesso` pair (OpenID Native SSO 1.0 draft 07), and `/hsk`, a probe of the server's
     * Hardware Security Key admin API that previously had no client caller at all.
     *
     ...
     */
    expect(allSectionsFlat.length).toBe(24);
    ...
    expect(allSectionsFlat.map((s) => s.id)).toContain('hsk');
  });
});
