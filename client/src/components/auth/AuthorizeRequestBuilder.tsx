            ...
            {customs.map((custom) => (
              <div key={custom.id} className="flex gap-2 items-center">
                ...
                <button
                  ...
                  className="p-1 rounded text-muted-foreground hover:text-red-400 bg-transparent border-none cursor-pointer shrink-0"
                >
...
function Warning({ children, tone = 'warn' }: { children: React.ReactNode; tone?: 'warn' | 'info' }) {
  return (
    ...
    >
      <AlertTriangle
        className={cn('h-3.5 w-3.5 mt-0.5 shrink-0', tone === 'warn' ? 'text-amber-400' : 'text-sky-400')}
      />
      <p className={cn('text-[0.7rem] leading-relaxed', tone === 'warn' ? 'text-amber-200' : 'text-sky-200')}>
        {children}
      </p>
    </div>
  );
    ...
    <div className={cn('px-3 py-2', !enabled && 'opacity-55')}>
        ...
        <div className="ml-auto flex items-center gap-1 shrink-0">
          {onRegenerate && (
            <button
              ...
              className="p-1 rounded text-muted-foreground hover:text-indigo-300 bg-transparent border-none cursor-pointer"
            >
      ...
      {enabled && (
        <div className="mt-1.5 pl-5">
          ...
          {error && <p className="text-[0.65rem] text-red-400 mt-1">Invalid JSON: {error}</p>}
        </div>
      )}
    </div>
  );
