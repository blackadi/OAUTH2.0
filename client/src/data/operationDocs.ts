...
const docs: Record<string, Record<string, OpDoc>> = {
  ...
  mcp: {
    ...
    'resource-metadata': {
      ...
      tips: "There is nothing to generate here, and nowhere in Authlete to look it up: this value is simply the address of a real OAuth resource server, not a credential or an identifier Authlete issues. Authlete has no RFC 9728 feature at all — unlike CIMD below, no service flag, no console screen, nothing to be \"compliant\" with on Authlete's side. Two things you can actually point this at: this deployment's own origin (already filled in above, since it does implement this) to see this server's own document — built from its regular discovery data, not stored anywhere special — or any other real server that implements RFC 9728. Before trusting either, check it yourself: open {origin}/.well-known/oauth-protected-resource directly in a new browser tab first. Real JSON means the origin is right; an HTML page (or anything else) means the wrong host, and this tool will now say so rather than pretending it worked.",
    },
    cimd: {
      ...
      tips: 'Nothing here is issued or generated by Authlete, and there is no console screen for it — you make this URL yourself; Authlete only ever reads it. It is three steps: host a small JSON file anywhere that serves HTTPS (a GitHub Gist\'s own "Raw" link needs no server of your own); note the exact URL that link gives you; then edit the JSON so its own "client_id" field equals that exact URL, character for character — a document that does not name itself is rejected no matter what else is right, and this is the single most common mistake. Past that it is shape: HTTPS required, a real path component (not a bare host), no client_secret_basic/post/jwt or a client_secret field at all (CIMD clients are necessarily public), and token_endpoint_auth_method set to "none". This tab never talks to Authlete — to see Authlete actually fetch and validate what you built, use the same URL as client_id in the Full Flow Wizard\'s Authorize step below.',
    },
    'authorize-url': {
