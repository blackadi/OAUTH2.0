...
- **The Auth Code (PKCE) panel could only ever authenticate via `client_secret_post`, and that was a gap, not a design choice** (2026-09-16). `CallbackPage.tsx`'s token exchange only ever put `client_secret` in the body — never `Authorization: Basic` — so a client registered `client_secret_basic` on Authlete could not be used from this panel at all, refused with `[A157357]`/`[A157304]` depending on what was sent. `ParSection.tsx` and CIBA had already solved this with a three-way `authMethod` selector (`'post' | 'basic' | 'none'`); the Auth Code panel was the one flow still missing it. Fixed by adding the same selector (`AuthorizationCodePanel.tsx`), a matching `ClientAuthMethod` type and `authzClientAuthMethod`/`activeClientAuthMethod` session keys (`session-keys.ts`), and branching `CallbackPage.tsx`'s exchange on it — `'basic'` builds an `Authorization` header via `basicAuthHeader()` (`http.ts`) and keeps the secret out of the body entirely, exactly as `par.service.ts`'s `doParRequest` already does for PAR. `TokenRequestPanel.tsx` had an unused `basicAuthClientId` prop built for exactly this display; it is now wired up. Default stays `'post'`, so every caller that predates the method (a session with only `authzClientId`/`authzClientSecret` set, no method) behaves exactly as before.
- **`StepUpSection`'s re-authorization always assumed the SPA's default public client — a bug independent of client type, found while testing the fix above** (2026-09-16). `handleReAuthenticate` hardcoded `client_id: CLIENT_ID` and unconditionally cleared `authzClientSecret`, so re-authorizing a token obtained via any *other* client (Client Credentials, Password, Refresh Token, or the Auth Code panel with a different client typed in) silently re-authorized as the wrong client instead. The app already tracks "which client does the current token belong to" — `SESSION_KEYS.activeClientId`/`activeClientSecret`, written by `CallbackPage.tsx` and `AuthFlowsSection.tsx`'s `saveClientCredentials` after every grant, already read correctly by `TokenOpsSection.tsx`. `StepUpSection` was the one place that did not read it back; it now does, plus the new `activeClientAuthMethod`, rather than gaining a fourth client-id/secret input panel of its own.
- **`activeClientSecret` was never written for the authorization_code grant at all, and that is what turned into a live `[A157304]`** (2026-09-16, the actual bug behind the two fixes above). `AuthFlowsSection.tsx`'s `saveClientCredentials` writes `activeClientId` **and** `activeClientSecret` for the other four grants; `CallbackPage.tsx` only ever wrote `activeClientId`. So a token obtained through the Auth Code panel with `client_secret_basic` selected — that part working correctly, `Authorization: Basic` sent and a `200` returned — still left `activeClientSecret` empty. `StepUpSection`'s re-authorization then correctly read `activeClientAuthMethod: 'basic'` but had no secret to send with it, so the retried token exchange sent **neither** a body secret **nor** a Basic header. Authlete's answer named the gap precisely: `[A157304] The client authentication method is 'client_secret_basic' but the request does not include a client secret.` — not a channel mismatch (`[A157357]`), an outright absence. Live-verified on `/step-up` against `https://oauth2-0-ekh2.onrender.com`, client `3848077211`. Fixed by having `CallbackPage.tsx` write `activeClientSecret` alongside the other two after every successful exchange (present only when `authMethod !== 'none'` and a secret was actually used — never the SPA's own empty default). No test in the repo checked `activeClientId`/`activeClientSecret` being written at all before this; `CallbackPage.test.tsx` now has two that do.
