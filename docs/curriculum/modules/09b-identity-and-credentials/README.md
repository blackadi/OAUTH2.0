...
| **OID4VCI** | Nine endpoints exist (`vci.routes.ts`) and delegate to Authlete. **Verifiable credentials are enabled *and* configured on this service** (2026-08-14, in two steps two days apart): the issuer metadata document is real and conformant, and the credential issuer now has a JWK Set, so `/vci/jwks` and `/vci/jwtissuer` answer **200**. The lab reads all three states of those endpoints — `NOT_FOUND` (feature off) → `INTERNAL_SERVER_ERROR` (on, no key) → `200`: **enabling a feature is not the same as configuring it, and configuring it is not the same as configuring it safely.** The one thing that can still be wrong returns 200, which is why the lab checks the *body* for private key members. Issuance itself is still not run here — it needs a wallet this repo does not contain. |
