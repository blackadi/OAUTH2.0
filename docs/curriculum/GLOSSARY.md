...
| **Unlinkability** | RFC 9901 §10.1 | Four distinct properties. Issuer/verifier unlinkability against a coerced verifier *"cannot be achieved"* in SD-JWT. Verifier/verifier unlinkability fails by default too, because the issuer-signed JWT is byte-identical across presentations. | Module 09b |
...
| **Fail-open auth** | *No spec term — failure pattern* | Authentication middleware that allows the request when its configuration is absent. `require-basic-auth.ts` **used to** return *allow* if `MGMT_CLIENT_*` were unset — fixed (`0229daa`): it now fails closed, with an error log per rejection and a startup warning. | Module 09b — Lab 7; Module 11 describes the fix |
