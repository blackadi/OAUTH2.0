...
| 7 | `prompt=none` returns a 302 with an **empty** `Location` header | 08 | ✅ reproduced at audit time: `HTTP/1.1 302 Found`, `Location: []` — exactly as described. **FIXED 2026-08-12**; now a real §3.1.2.6 error redirect (`login_required` with `state` and `iss`), or a code when the session satisfies the request. A **second, latent** finding was fixed with it: the `prompt=none` path fabricated `acr`/`auth_time`, so repairing this row alone would have armed a step-up bypass. See `PROGRESS.md`. |
