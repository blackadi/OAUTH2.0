...
|---|---|---|---|
| 9470-W1 | Correct the challenge status in the tutorial, and separate AS-to-RS from RS-to-client | S | ✅ **DONE 2026-08-14 (T2-11), exactly as specified.** Part 5 now opens with a two-participant sequence diagram and a six-row table separating **Response 1** (AS → RS, **403**, Authlete's `FORBIDDEN`, this repo's `introspection.controller.ts`) from **Response 2** (RS → client, **401**, RFC 9470 §3, *"your resource server — this repo does not implement one"*). The *"What the client learns"* table hangs off the 401 and is **split**: `acr_values`/`max_age` are §3's challenge parameters, while `acr`/`auth_time` are moved to a second table labelled *Response 1 only*, since they are this AS's addition and a client acting on them is trusting the AS's view of a token it already holds. **No code changed** — F-1's own *"being fair about what this endpoint is"* paragraph is why: the 403 is a defensible vendor mapping, and the defect was the conflation. **A fifth carrier turned up that no work item named** — see 9470-W7. |
