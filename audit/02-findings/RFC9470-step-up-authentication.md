...
**It is unreachable today, for a reason recorded elsewhere in the repo.** `docs/curriculum/PROGRESS.md:1242-1251`
...
| Nothing anywhere notes that the `prompt=none` step-up path fabricates the authentication event | all docs; `PROGRESS.md:1242-1251` records the dead code but not what it *does* | F-3 | **Omission** / **S2**, latent S1 |
...
- Repo-sourced live evidence: `docs/curriculum/PROGRESS.md:1242-1251` (`NO_INTERACTION` returns `responseContent: null` + ticket; the `:96` branch is unreachable)
