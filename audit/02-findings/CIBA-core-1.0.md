...
| CIBA-W3 | Use `parseBasicAuth` for CIBA client authentication | S | ✅ **DONE 2026-08-13 (T1-20).** Three channels, a unit test each, and `appendToParams` extracted to `utils/params.ts` so PAR and CIBA share one implementation rather than growing a second subtly-different copy — the failure mode this repo has already paid for four times with bearer parsers. **Verified live, and the negative control is the interesting half**: a Basic-authenticated request now reaches `USER_IDENTIFICATION` (previously impossible), while body credentials for that same `CLIENT_SECRET_BASIC` client now correctly earn `401 [A157357]`. The old code had been *silently converting* every request onto the Basic channel, so a `client_secret_post`-shaped request succeeded against a `client_secret_basic` client — precisely the guessing `par.service.ts`'s comment forbids. The SPA gained a **Client Auth Method** selector to match. |
| CIBA-W4 | Set `bcDeliveryMode` on one client | S | ✅ **DONE 2026-08-12 (T1-6)** — `POLL` on client `1523514379`. **The full sequence was run**: `USER_IDENTIFICATION` → `authReqId` (`expiresIn 600`, `interval 5`) → `authorization_pending` (400) → `complete` → `NO_ACTION` → token + ID token → replay refused `invalid_grant`. Three things worth carrying: **`NO_ACTION` is success in poll mode**, not a no-op, so a reader who assumed otherwise would stop one step short of the token; **`authorization_pending` is a 400**, so a polling loop must not treat 400 as terminal; and the **`auth_req_id` is single-use**, which answers Exercise 3d's open question. This closes F-4's *no CIBA flow can complete* for one client — the other three still have no `bcDeliveryMode`. |
| CIBA-W5 | State the wire-format gap until W1/W2 ship | S | ◻ **Tutorial half DONE 2026-08-14 (T2-1); the `AGENTS.md` half stays in T2-15.** `CIBA-TUTORIAL.md` states both departures as a table: §7.1's form-encoded request versus the JSON `parameters` envelope, and §7.3's `auth_req_id` **from the backchannel endpoint itself** versus a `ticket` plus a second call to `/api/ciba/issue`. It also records that `/api/ciba/issue` authenticates nobody. **Two things found while writing it, neither in the criterion.** `expires_in` read 3600 against a live **86400**. And every worked example passed `clientId`/`clientSecret` in the **body**, while the only client on this deployment with `bcDeliveryMode` set is `CLIENT_SECRET_BASIC` — the method Part 2 *recommends*, citing Authlete's own guide — so **the tutorial's examples 401 with `[A157357]` against the configuration the tutorial tells you to build.** The Basic form is now primary. That also makes Part 6's *"Wrong Client Secret"* demo prove nothing: body credentials are refused for the **channel** before the secret is read, so a negative test passed for the wrong reason. |
