...
| 7592-W3 | State plainly that RFC 7592's HTTP surface is not implemented | S | ✅ **DONE 2026-08-14 (T2-15).** `SPEC-INVENTORY.md`'s RFC 7592 row and the note under it now separate the two claims exactly as asked: every RFC 7592 **operation** is reachable, and **none of its HTTP surface is** — no per-registration client configuration endpoint, no `GET`/`PUT`/`DELETE`, no `registration_client_uri` in the registration response; four `POST` routes taking the registration access token in a **JSON body** instead. **The reason this needed saying now rather than earlier is the part worth keeping:** 7591-W1 shipped on 2026-08-14 and made `register` return §3.2.1's response *as the body*, so the RFC 7591 half became genuinely conformant — which makes it much easier to read the pair as done. **The body is conformant; the endpoint is not**, and the row states both rather than one. |
