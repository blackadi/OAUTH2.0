...
|---|---|---|---|
| 7591-W1 | Return `responseContent` **as** the body | S | `POST /api/client/dcr/register` 201 body is the client information response with `client_id` at the top level; `resultCode`/`resultMessage` are not exposed. Update the four controller tests. |
