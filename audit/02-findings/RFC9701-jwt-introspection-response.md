...
| 9701-W4 | Add RFC 9701 to `SPEC-INVENTORY.md` and name it in Module 10 | M | ✅ **DONE 2026-08-14 (T2-5), both halves.** Fetched `rfc-editor.org/info/rfc9701`: *"JSON Web Token (JWT) Response for OAuth Token Introspection"*, **Proposed Standard**, **January 2025**, obsoleting and updating nothing. Row added to `SPEC-INVENTORY.md` §4 directly beneath RFC 7662, which is the right adjacency — it is the same question answered in a different envelope. Module 10's *FAPI 2.0 Message Signing* section now names it for the signed-introspection element and it joins *"The table to internalise"*. **The `rsUri` requirement is carried into the module deliberately**, framed as *vendor requirement, normative purpose*: Authlete demands it, the RFC does not, and the reason it exists is that `rsUri` becomes the **`aud`** — a signed statement addressed to nobody is poor evidence. That framing is the module's own recurring lesson, so it reinforces rather than interrupts. |
