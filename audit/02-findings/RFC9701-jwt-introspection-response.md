...
|---|---|---|---|
| 9701-W1 | Handle `action: "JWT"` in `introspection-standard.controller.ts` | S | `JWT` → 200 with `Content-Type: application/token-introspection+jwt` and `responseContent` as the body. Unit test asserts status, content type and body for all four action values. |
...
| 9701-W5 | Add a `curl` recipe exercising the JWT form | S | `CURL-TEST.md` gains an `Accept: application/token-introspection+jwt` example whose documented output matches reality after 9701-W1 |
