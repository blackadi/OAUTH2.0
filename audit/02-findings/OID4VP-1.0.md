...
| VP-W2 | State the no-obligations fact in the inventory row | S | ✅ **DONE 2026-08-14 (T2-5), in *two* rows — the §9b row and the *Not-yet-in-repo* row, which was the one actively misreporting.** That second table is headed *"gaps this curriculum addresses"* and listed OID4VP under it as *"No verifier implementation"*, i.e. exactly the unbuilt-feature reading this item removes; it now says **"Not a gap."** Provenance: `openid.net/specs/openid-4-verifiable-presentations-1_0.html`, *"OpenID for Verifiable Presentations 1.0"*, **Final**, **9 July 2025** — date **already correct**. **One correction to DR-13's wording**, which says the roles *"are Wallet and Verifier"*: the fetched document defines **three** — Wallet, Verifier and Credential Issuer — plus the Holder. The conclusion is untouched, since the Credential Issuer appears only as whoever issued the credential being presented, but the row says so rather than repeating a two-role claim the source does not make. |
