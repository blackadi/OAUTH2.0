...
Both were **verified live** by the repo (`PROGRESS.md:1123-1138`, `modules/08…/lab.md:638-660`). With
...
**The fix is small and the repo already knew it.** The `PROGRESS.md` entry as it stood on 2026-08-10 read *"Fix is one line — exact comparison against a registered set."* **That sentence no longer exists**: the entry was rewritten when the fix shipped, and now records the fix instead (`PROGRESS.md:1123-1138`, with both fixes recorded at `:1404` — the 2026-08-10 origin comparison and its supersession by T0-4). Quoted here from the pre-fix revision (`git show b5e60d4~1:docs/curriculum/PROGRESS.md`, line 401) because the prediction is part of the finding's evidence. Note the contrast that same entry draws, which survives the rewrite at `PROGRESS.md:1128-1129`: the **authorization** endpoint gets exact matching right (400, no `Location`). Two redirect-validating code paths, one correct.
...
| Open redirect found, two payloads, "survives `NODE_ENV=production`", "Fix is one line — exact comparison against a registered set" | `PROGRESS.md:1123-1138`; `modules/08…/lab.md:638-660` | Confirmed by reading the code; matches §3 | **Accurate — and the analysis is better than the code** |
...
- Repo-sourced live evidence: `PROGRESS.md:1123-1138` (both bypass payloads, verified), `modules/08…/lab.md:638-660`
