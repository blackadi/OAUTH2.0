...
- **Repo docs under test:** `docs/curriculum/SPEC-INVENTORY.md`, `docs/curriculum/PROGRESS.md:1097-1103`, `docs/curriculum/modules/09b-identity-and-credentials/`
...
4. Docs: `PROGRESS.md:1097-1103` diagnoses this precisely, including that the SDK types the field as optional so
...
No `requestBody`. `PROGRESS.md:1097-1103` records the diagnosis and I confirm it against the code and the SDK:
...
first. `PROGRESS.md:1110-1112` groups this with two siblings — Module 06's Zod failure and Module 08's unset
...
|---|---|---|---|
| The endpoint is broken, the SDK's optional typing hid it, `{}` returns 200, and the failure misreports the cause | `PROGRESS.md:1097-1103` | **Confirmed** line by line. An unusually complete diagnosis | **Accurate** |
...
| `POST /api/federation/registration` is written correctly | `PROGRESS.md:1112` | Confirmed — it forwards `entityConfiguration` / `trustChain` (`services/federation.service.ts:34-37`) | **Accurate** |
...
- Repo-sourced live evidence: `PROGRESS.md:1097-1103` (`[A126203]`, and the direct `{}` call returning 200)
