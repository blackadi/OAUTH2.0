...
silent renewal is impossible and the failure is unattributable. `PROGRESS.md:530-531` puts it well — *"breaks
...
## Finding F-2 — the advertised ID Token signing algorithms omit RS256 (S3)

Live: `id_token_signing_alg_values_supported = ["HS256","HS512","ES256","HS384"]`.
...
verified by any third party — acceptable for a confidential-client demo, and the reason Module 08's asymmetric
validation branch (`lab.md:365-399`) remains unexercised.
...
`PROGRESS.md:1076` records this as *"flagged as a Module 07 report item rather than a Module 08 finding"*, which
...
| OIDC-W5 | Apply or retire the two divergent flags | S | `idTokenAudType` and `idTokenReissuable` either set to `AGENTS.md`'s recommended values, or the table notes that this deployment deliberately differs — and, for `idTokenReissuable`, that the handled action is unreachable while it is `false`. |
