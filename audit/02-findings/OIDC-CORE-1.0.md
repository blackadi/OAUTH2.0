...
| 9 | `claims` request parameter with essential claims | §5.5 | ⚠️ **This ✅ was wrong, corrected 2026-09-01.** The original note — *"consented claims forwarded"* — checked that claim **names** reached Authlete. They did. What it did not check is that claim **values** did, and they did not: `authorization.controller.ts` passed `result.idTokenClaims`, the claims *request* JSON, as `AuthorizationIssueRequest.claims`, the claim *values*. In OIDC's claims-request syntax `{"name":null}` means *"no special requirements"*; Authlete embedded it verbatim, so **every id_token carried `"name": null`** while `/api/userinfo` returned real values for the same claims. Found by `fapi2-security-profile-final-test-claims-parameter-identity-claims` — 22 failures on 116 successes, the giveaway being *"Value of name **differs** between id_token and userinfo"* (`differs`, because a null claim is present-but-invalid). Now fixed: values come from `utils/demo-claims.ts`, shared with the userinfo endpoint so the two cannot disagree. **The lesson for this audit's method:** "the field is populated" is not "the field is populated with the right kind of thing", and no gate in this repo could see the difference — the type is `string` either way. |
