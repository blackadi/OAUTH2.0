...
| CIMD-W2 | Decide whether to enable the flag | S | ✅ **DECIDED + EXECUTED 2026-08-14 (DR-05) — enabled.** `clientIdMetadataDocumentSupported` is now `true`; `client_id_metadata_document_supported` appears in discovery. **The MCP claim stays qualified**, as a separate half of the same ruling — `MCP-OAUTH-TUTORIAL.md` still does not say "out of the box" unconditionally, since CIMD alone doesn't fix OAuth 2.1 conformance or the issuer/host question (that half is DR-11). See `05-decision-records.md` DR-05. |
