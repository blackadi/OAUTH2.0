...
| 4 | **Exact** `redirect_uri` matching against pre-registered URIs | Authlete's, and verified: the authorization endpoint rejects a non-matching URI with 400 and no `Location` (`PROGRESS.md:1523-1524`) | ✅ |
...
- Repo-sourced live evidence: `PROGRESS.md:1523-1524` (exact redirect-URI matching at the authorization endpoint), `modules/08…/lab.md:94-107,148-173` (`sub` in the ID token)
...
| FAPI1-W2 | Make `computeFapiMode` total over Authlete's `fapiModes` | S | ✅ **DONE 2026-08-14.** Domain is now `sp` · `ms` · `fapi1-advanced` · `fapi1-baseline` · `unknown` · `disabled`, total over SDK 1.0.0's six-member `FapiMode`. **The `unknown`/`disabled` split is the half that is not about FAPI 1.0**: `"disabled"` is a statement about configuration (no mode set), `"unknown"` about recognition (a mode set that this server does not know), and collapsing the second into the first asserts a posture nobody checked — FAPI2-W1's defect one layer down. A seventh Authlete member now lands visibly. Within a generation the stronger mode wins (message signing over security profile, Advanced over Baseline) because `fapiModes` is a list; FAPI 2.0 behaviour is unchanged, the previous three-branch form being equivalent. `specs.securityProfile` follows `mode` rather than being the constant `"FAPI 2.0 Security Profile"`, which was wrong the moment `mode` could be a FAPI 1.0 value. Consumers moved in the same commit: `openapi.routes.ts`, `FapiSection.tsx`, `operationDocs.ts`, `docs/FAPI-TUTORIAL.md`. **No curriculum transcript changed** — `fapiModes` is absent on this service, so the live answer is still `mode: "disabled"`. Test per enum member plus both edge states, in `tests/unit/routes/fapi.routes.test.ts`. |
