...
|---|---|---|---|
| PS-W1 | Add a paragraph to Module 04 and a `SPEC-INVENTORY.md` row | S | ✅ **DONE 2026-08-14 (T2-16).** Module 04's paragraph sits directly beside the `scopes_supported` metadata block — the right adjacency, since the finding *is* about that member — and names all three things the criterion asked for: the **`regex`** scope attribute, the **`dynamicScopes`** response field, and the discovery limitation. The `SPEC-INVENTORY.md` row is in the new *vendor features* section. **The framing that came out of writing it is the durable part:** `scopes_supported` can list literal strings and has no member for a pattern, so **a client that discovers this AS correctly can never use the feature while a client that hardcodes the value can** — the exact inverse of the advice every other module gives. |
| PS-W2 | Add "accepted but unadvertisable" to Module 09a's taxonomy | S | ✅ **DONE 2026-08-14 (T2-16), and it earned a four-row table rather than a sentence.** The three existing states all describe a capability that is *listed* and delivers less than the listing implies; this one is the **inverse**, and the table's last column makes that legible by naming what misreading each state costs — for this one, *"a capability you never knew you had"*. **The row that makes the taxonomy click:** in a capability matrix, accepted-but-unadvertisable does not look like a green tick, it looks like the feature is **absent**. Also notes that this repo cannot demonstrate it — no scope-management endpoint — and that *that* limitation is itself an instance of the same shape. |
