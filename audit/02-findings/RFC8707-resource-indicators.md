...
|---|---|---|---|
| 8707-W1 | Add automated coverage for `resource` → `aud` | S | ✅ **DONE 2026-08-14 (T2-17).** Three integration tests in `tests/integration/routes.test.ts`. **Scoped honestly**: Authlete is mocked in that suite, so the `resource` → `aud` *mapping* is the vendor's and stays verified live in `modules/04…/lab.md` Exercise 4. What the tests pin is the half that is ours and the half that has actually broken — that `resource` crosses the boundary into Authlete's `parameters` verbatim (it survives only because `parameters` is `req.rawBody`, so **no code names the parameter** and nothing would notice it going missing), that `invalid_target` is passed through unrewritten, and that an `aud` in an introspection response reaches the caller. |
...
| 8707-W3 | Exercise the multi-value and query-component cases | S | ✅ **DONE 2026-08-14 (T2-17), read-only.** Both **accepted**, and the AS echoes both values back verbatim including the query component: `resources: ["https://api.example.com/orders?v=1", "https://api.example.com/payments"]`. Two controls prove the checks are live rather than absent — a fragment gives `[A251308]`, a relative reference `[A251307]`. §2's rule set is complete: a query component is permitted, a fragment is not, and Authlete implements exactly that line. **The multi-value result is the one worth having** — an AS that silently kept only the first value would audience-restrict a token to half of what was requested, with no error anywhere. Evidence: `SERVICE-CONFIG-PROBE.md` §22.1. |
