...
> - **F-3 (the missing `SPEC-INVENTORY.md` row) is done, not open — this bullet was never updated after
>   `ATT-W1` shipped 2026-08-14 (T2-5).** `SPEC-INVENTORY.md` has carried the row since, recording the
>   method as *"withdrawn here, deliberately"* — the *"permitted but not configured"* → *"declined"*
>   transition Module 09a's taxonomy has vocabulary for. See `04-remediation-plan.md`'s `ATT-W1` row.
>
...
- **Repo docs under test:** `AGENTS.md` (mentions the headers in the PAR known-gap note), `docs/curriculum/SPEC-INVENTORY.md` (row added 2026-08-14, `ATT-W1` — this line predates that and is stale)
...
|---|---|---|---|
| ATT-W1 | Add a `SPEC-INVENTORY.md` row | S | ✅ **DONE 2026-08-14 (T2-5).** Fetched `datatracker.ietf.org/doc/draft-ietf-oauth-attestation-based-client-auth/`; *"OAuth 2.0 Attestation-Based Client Authentication"*, Active Internet-Draft, revision **10**, **2026-07-06**, expires 7 Jan 2027, intended status Standards Track — revision and date confirmed exactly. Row added to §5, beside RFC 8705, since both are client-authentication methods. **One clause of the criterion is now false and was corrected rather than copied:** it says the method is *"advertised but unconfigured"*, but **T1-5/DR-07 withdrew `ATTEST_JWT_CLIENT_AUTH` on 2026-08-12**, so it is no longer advertised at all. The row reads *"withdrawn here, deliberately"* and names the withdrawal. The SDK's citations on `TokenRequest`/`PushedAuthorizationRequest` are noted as the criterion asked. |
