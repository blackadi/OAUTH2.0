...
| **VCI-W6** | Set a credential-issuer JWK Set, or document the boundary | S | ✅ **CLOSED 2026-08-14 — the operator set `credentialJwks`, and all three checks pass.** One EC P-256 key, `kid: vc-issuer-1`, `alg: ES256`. Verified live against the deployment: `GET /api/vci/jwks` → **200**, `GET /api/vci/jwtissuer` → **200** (the same key set wrapped in an `issuer`), and — **the check that mattered** — both publish the **public half only**. The stored service value contains the private scalar `d`, because the issuer must sign with it; Authlete strips it on the way out. Confirmed by parsing the response and asserting on `d`/`p`/`q`/`dp`/`dq`/`qi`/`k`, **not** by reading the 200: a JWKS endpoint echoing `d` would publish the key every credential it signs is verified against, and would still answer 200. **Module 09b Exercise 7 was rebuilt a second time and is better again** — the two-state *"enabling is not configuring"* lesson becomes **three dated states** of the same endpoints (`NOT_FOUND` → `INTERNAL_SERVER_ERROR` → `200`), each transition naming a different missing value, plus the private-member check. **And closing the gap improved a diagnosis rather than deleting one:** the `UNVERIFIED` marker on issuance used to blame the missing JWK Set, and that was *hiding the fact that nothing else was missing* — issuance now needs only a **wallet this repo does not contain**, which tells a reader what to build instead of what to switch on. |
