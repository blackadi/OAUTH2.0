...
- enables the **password** grant (`PASSWORD`; RFC 9700 §2.4 MUST NOT, removed in OAuth 2.1) — and `PROGRESS.md:2087-2088` records a live transcript issuing a 24-hour access token and a 10-day refresh token from it;
...
| MCP-W3 | Qualify or scope the "out of the box" claim | S | ✅ **DONE, in three parts and the last one closes it.** The tutorial's claim went in T0-5 (2026-08-11), replaced by a precondition table; the table was re-verified and the CIMD/issuer rows flipped to ✅ when DR-05 and DR-11 landed (2026-08-14); and **T2-8 added the `README.md` row this item's other half needed** — *MCP / CIMD: **Partial***, naming `clientIdMetadataDocumentSupported: true` and the two things that still block MCP end to end (OAuth 2.1's requirement to reject `implicit` and `password`, both enabled here deliberately, and the absent `registration_endpoint`). **The index half is also done**: `docs/README.md` gained rows for `MCP-OAUTH-TUTORIAL.md` and `STEP-UP-AUTH-TUTORIAL.md` in T2-1 — both had been orphaned from the tutorial index. Note the doc-delta row above records that `README.md` never carried the *original* claim, so this was never a correction there, only an addition. |
