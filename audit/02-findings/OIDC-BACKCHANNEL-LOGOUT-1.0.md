...
when the `issuer` and `audience` options are passed, and they are not. `PROGRESS.md:552-553` states this exactly:
...
`PROGRESS.md:548-553` records this with the server log confirming the real cause
...
| Back-channel logout receipt cannot work (`JWKS_URI` unset), misreports why, and never checks `iss`/`aud` | `PROGRESS.md:548-553` | Confirmed line by line | **Accurate** |
...
| BCL-W3 | Distinguish server misconfiguration from a bad token | S | Unset `JWKS_URI` yields 500 `server_error` with an operator-facing log, not 400 `invalid_request`. Same fix pattern as the two sibling instances `PROGRESS.md:519-521` names. |
