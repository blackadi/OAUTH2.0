...
when the `issuer` and `audience` options are passed, and they are not. `PROGRESS.md:1538-1539` states this exactly:
...
`PROGRESS.md:1534-1539` records this with the server log confirming the real cause
...
| Back-channel logout receipt cannot work (`JWKS_URI` unset), misreports why, and never checks `iss`/`aud` | `PROGRESS.md:1534-1539` | Confirmed line by line | **Accurate** |
...
- Repo-sourced live evidence: `PROGRESS.md:1534-1539`
...
| BCL-W3 | Distinguish server misconfiguration from a bad token | S | ✅ **DONE 2026-08-13**, with T1-14/T1-15 since it is the same function. Unset `JWKS_URI` — or either new expectation — yields **500 `server_error`** and an operator-facing log naming which knob is missing. **The check runs before the token is parsed**, which is stronger than the criteria asked: a server that cannot verify a signature must not render *any* verdict on the token, so even the `events`-claim rejection became a 500 rather than a true-but-unearned 400. `FED-W5` is the same bug in the federation service and stays in **T1-18**. Same fix pattern as the two sibling instances `PROGRESS.md:1505-1507` names. |
| BCL-W4 | Configure `JWKS_URI`, or state that receipt is not runnable | S | ✅ **CLOSED 2026-08-14 (T2-17) — as the second option, which is now a much smaller claim than when it was written.** Receipt is **runnable and was run**: T1-14 stood up a local JWKS and drove all seven validation cases through the live server. What is unset is a *permanent* `JWKS_URI` for a *specific* peer OP, and there is no peer OP — this deployment is nobody's RP. Configuring one means naming a real upstream, which is a deployment decision and not a repo one, so it cannot be "done" here in any meaningful sense. The endpoint states its own condition: a **500 naming the missing setting** rather than a 400 blaming the caller's token. *(Original note follows.)* ⚠️ **Half done 2026-08-13 — the tutorial now says so plainly**, and the endpoint says so itself with a `500` naming the missing setting rather than blaming the token. **The receipt path was also proven runnable**: T1-14's verification stood up a local JWKS and drove all seven cases through the live server, so "not runnable" is now "not configured", which is a different and much smaller claim. Configuring it permanently is a deployment decision, not a code one. |
| BCL-W5 | Advertise the capability and register one `backchannel_logout_uri` | S | ✅ **DONE 2026-08-15 (T2-17).** The block lifted when **T0-2** made `id_token_hint` a verified assertion on 2026-08-11 — until then a hand-made JWT naming any subject was a forced-logout primitive, and registering a URI would have armed it. `backchannelLogoutSupported: true` (discovery now carries `backchannel_logout_supported: true`, 65 members) and `1523514379` gained a `backchannel_logout_uri`; both writes `200`, **0 unexpected field changes**, and the `tokenAuthMethod` / `authorizationSignAlg` canaries held against `client/update`'s replace semantics (CU-W1). `backchannel_logout_session_supported` stays **absent**, consistent with DR-08 declining Session Management and `sid` together. ⚠️ **The registered URI is this deployment's own receiver — a loopback**, because there is no third-party RP. F-4's delivery path is therefore **demonstrable, not interoperable**, and must not be written up as *"back-channel logout works"*: that is the *advertised but unusable* shape this audit found four times, and discovery is precisely where it would mislead. Evidence: `SERVICE-CONFIG-PROBE.md` §23.2. |
| BCL-W6 | Replace the duplicate client-listing `fetch()` with `authleteApi.client.list` | S | ✅ **DONE 2026-08-14.** The only remaining raw `fetch()` to Authlete is the justified one — `callAuthleteIssueToken`, because SDK 1.0.0 exposes no backchannel logout token API. **`AGENTS.md` now names the exempt *call* rather than the file**: one legitimate exception in a file makes the next one look like house style, which is how this duplicate survived. **The item turned up a finding larger than itself.** The compiler refused `client.backchannelLogoutUri` — **SDK 1.0.0's `Client` model does not have that field**, carrying 104 of Authlete 3.0.16's 108 properties (the omissions: `backchannelLogoutUri`, `backchannelLogoutSessionRequired`, `spiffeId`, `spiffeBundleEndpoint`). `AGENTS.md` said the SDK *"silently strips"* unmodelled members, which would have made this migration deliver logout tokens to **nobody**, silently, while answering 200. **That claim is true of `Service` and false of `Client`**: `Service$inboundSchema` is a plain `z.object`, `Client$inboundSchema` wraps itself in `collectExtraKeys$` and collects into `client.additionalProperties`. Established by parsing a fixture through the real schema, not by reading the model — and locked by a test that does the same, so an SDK bump fails loudly instead of emptying the delivery list. **Rule recorded in `AGENTS.md`: do not generalise one SDK model's tolerance to another.** 6 cases added in `tests/unit/services/backchannel-logout.service.test.ts`, which previously exercised `issueAndDeliverToAll` not at all. |
