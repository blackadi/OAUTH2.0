...
|---|---|---|---|
| HSK-W1 | Add the four endpoints to `docs/API.md` | S | ✅ **DONE 2026-08-14 (T2-16).** All four documented with request shape, required-vs-optional fields (`kty` and `hsmName` required), the full action→status map, and the admin-auth requirement **including that it fails closed**. `DELETE` gets its own ⚠️ box, and it makes a point the criterion did not: **deleting a handle is not deleting a key** — the key material lives in the HSM and this API never sees it, which is the whole reason for the indirection. |
| HSK-W2 | Write a short `docs/` page and a `SPEC-INVENTORY.md` row | S | ✅ **DONE 2026-08-14 (T2-16), with one deliberate departure: no separate `docs/` page.** The row landed in a **new `SPEC-INVENTORY.md` section — *"Vendor features — implemented here, defined by no specification"*** — carrying all three of HSK, parameterized scopes and `attributes` together, which is the treatment the criterion asked for and puts them in one place rather than three. The section states the reason they belong in a specification inventory at all: *the most useful thing to know about each is that there is no RFC to check it against.* HSK's connection to Modules 00 and 05 is a cross-reference on the row and in the `docs/API.md` section; **a whole page would have been three paragraphs of prose about four endpoints nothing else consumes.** If HSK ever gains a consumer, promote it. |
