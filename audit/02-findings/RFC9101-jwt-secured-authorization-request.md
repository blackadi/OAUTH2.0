...
| 9101-W4 | Say in Module 05 which JAR halves this deployment can and cannot run | S | ✅ **DONE 2026-08-14 (T2-15) — and the second clause was stale.** It reads *"object signing symmetric-only until W3"*; **W3 shipped 2026-08-12**, so JAR **by value** now runs *asymmetrically* — client `2176571218` carries `requestSignAlg: ES256` and a registered JWK Set, and an ES256-signed request object validates against a real key. Only **by reference** is still unavailable: no client registers a `requestUris` entry and `require_request_uri_registration` is in force (9101-W2 open). Module 05 states both in a four-row table alongside PAR's two halves, and adds a distinction the item did not ask for: **JAR's §5.2 `request_uri` is a different artefact from PAR's** — the client hosts one, the AS mints the other, and conflating them is a common reading error. |
