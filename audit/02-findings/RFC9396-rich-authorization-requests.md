...
|---|---|---|---|
| 9396-W1 | Register one `authorization_details` type on the service and one client | S | A RAR request with that type is accepted; `authorization_details_types_supported` appears in discovery; the consent screen renders the object; the granted details come back in the token response and on introspection. Retires one `UNVERIFIED` marker and exercises F-2's code for the first time. Console change only. |
...
| 9396-W4 | Audit `RAR-TUTORIAL.md` against real behaviour | M | Phase 3 work. Any transcript in it predates a runnable configuration, so each must be re-run after W1 or labelled. |
