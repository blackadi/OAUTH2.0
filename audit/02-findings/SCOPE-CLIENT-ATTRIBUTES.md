...
|---|---|---|---|
| ATTR-W1 | Validate and type the `attributes` field | S | A Zod shape for the key/value pair array replaces `as any`; an invalid shape is rejected with 400 rather than forwarded. Unit test covers accept and reject. |
| ATTR-W2 | Document `attributes` in `docs/API.md` | S | Shape, an example, and a note that Authlete assigns meaning to some keys — with `regex` on *scope* attributes as the example of why the namespace is not inert. Pairs with **HSK-W1**. |
...
| ATTR-W4 | State that this repo manages no scopes | S | One line in `docs/API.md` or Module 04, naming the two Group B features it blocks. |
