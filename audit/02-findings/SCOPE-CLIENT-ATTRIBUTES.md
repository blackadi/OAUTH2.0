...
| ATTR-W3 | Add `SPEC-INVENTORY.md` rows for both features | S | ✅ **DONE 2026-08-14 (T2-16); confirmed and marked in T2-5's coverage sweep.** Both features sit in the *"Vendor features — implemented here, defined by no specification"* section alongside HSK, exactly as the criterion asked — one section rather than three scattered rows, stating why they belong in a specification inventory at all: **the most useful thing to know about each is that there is no RFC to check it against.** The item was satisfied by T2-16's single edit but never marked, so it read as open. **Two of the fifteen IDs in this batch were in that state** (see NSSO-W3) — a reminder that "shipped inside another item" and "recorded as shipped" are different facts, and only the second is visible to whoever plans the next session. |
