...
4. Docs: Module 05 Exercises 4–5 are accurate and reproducible. `AGENTS.md`'s **DPoP nonce flow** bullet and `FAPI-TUTORIAL.md` are
...
| 9449-W5 | Correct `AGENTS.md`'s **DPoP nonce flow** bullet | S | ✅ **DONE 2026-08-14 (T2-14), all four clauses.** The bullet now carries a two-row table — **AS: 400 `use_dpop_nonce` + `DPoP-Nonce`** (§8, quoted verbatim) versus **RS: 401 + `WWW-Authenticate: DPoP` + `DPoP-Nonce`** (§9) — states that a stale or mismatched nonce is refused with **`use_dpop_nonce`, not `invalid_dpop_proof`**, and reserves `invalid_dpop_proof` for a genuinely malformed proof. **The `FAPI-TUTORIAL.md` pointer was *made good* rather than dropped**, with the caveat that its nonce material is now marked `UNVERIFIED` (T2-1) for the same reason this bullet needs one: `dpopNonceRequired` is `false`, so none of it is exercisable here. **Why §8's 400 is not a detail, and the reason this was worth an S3 rather than an S4:** a client that retries only on a 401 never retries at the token endpoint at all — the wrong status does not merely mislabel the error, it stops the nonce dance from starting. |
| 9449-W6 | Produce one real nonce transcript | M | ✅ **DONE 2026-08-15 (T2-17)** — first option, then reverted. `dpopNonceRequired: true` + `dpopNonceDuration: 300`, three token calls, restored with **0 unexpected field changes**. **It confirms both halves of 9449-W5's correction, which had been made from the specification alone**: no nonce → `BAD_REQUEST` → **HTTP 400** with `error: use_dpop_nonce` **and** a `DPoP-Nonce` header; that nonce replayed → `OK`; a bogus nonce → `use_dpop_nonce`, **not** `invalid_dpop_proof`. **Three things the RFC does not say**: the nonce is **time-based, not one-time** — the same value came back on all three calls including the success, so a client caches it for `dpopNonceDuration` and one written to treat repetition as replay would be wrong; a nonce **is** returned on success at the token endpoint; and Authlete's `[A254307]` says *"different from the expected one"* **even when no `nonce` claim was sent**, so the message misdirects on first contact while the `error` code stays correct. The relay was already right — `token.controller.ts:69` sets the header **before** the `switch`, so every branch carries it. Live posture unchanged (`false`). Transcript: `SERVICE-CONFIG-PROBE.md` §23.1. |
| 9449-W7 | Enable `dpopRequired` on the `DPOP` client | S | ❌ **DECLINED 2026-08-14 (T2-17) — the criterion names a client it does not know the second job of.** `1678274156` really is named `DPOP` (confirmed live, `PUBLIC`, `dpopRequired=false`), which is why this row reads as obvious. **It is also `$PUB_CLIENT_ID`**, and `AGENTS.md` pins it, with `1523514379`, as *curriculum infrastructure*: PKCE is deliberately unenforced on both because **Module 02 teaches the plain code flow and Module 03 shows what it costs**, and the lesson needs a client that still permits it. Setting `dpopRequired` makes **every** token request from that client fail without a proof, so Module 03's transcripts stop reproducing — and **nothing in the build, the tests or `check-docs` would say so, because labs are prose.** That is the exact failure DR-03 caused on 2026-08-14 and the reason `AGENTS.md` now carries three searches for configuration changes. **The correct fix is a fifth client**, not a flag on this one — the same judgement T1-3 made when it created `2176571218` rather than converting an existing client, and for the same reason: the other four are load-bearing. Not scheduled; the demonstration is worth less than the module. **If it is ever done: create, do not convert.** |
