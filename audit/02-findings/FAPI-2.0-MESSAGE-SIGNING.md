...
|---|---|---|---|
| MS-W1 | Handle `StandardIntrospectionResponseAction.JWT` | S | ✅ **DONE 2026-08-13** — see `RFC9701-jwt-introspection-response.md` **9701-W1** for the detail, including the two reachability prerequisites the criteria did not name. **The profile's only live 500 is closed**, and §5.5.1's *shall* (an issued JWT introspection response is signed) is now demonstrably met: `alg: RS256`, not `none`. |
...
| MS-W3 | ~~Drop `none` from the introspection signing algorithms~~ → **document that it cannot be dropped** | S | ⚠️ **Not a console change — there is no console setting** (established 2026-08-12, T1-13). `introspection_signing_alg_values_supported` is derived from the service JWK Set plus an unconditional `none`; no Authlete 3.0 `Service` property lists it, and the `Client` schema has **no** introspection signing property either, so neither side can narrow it. Writing the only candidate (`introspectionSignatureKeyId`) changed the list but left `none` (`SERVICE-CONFIG-PROBE.md` §19). Folded into **JOSE-W2** as documentation. The §5.5.1 `shall` is unaffected: it requires *issued* JWT introspection responses to be signed, which is MS-W1's territory, not the metadata's. |
