...
| `HTTP/1.1 201 Created` / `DPoP-Nonce: <serverNonce>` / `{"requestUri":"…","expires_in":90}` | `FAPI-TUTORIAL.md`, **Part 4 → Step 3: Push Authorization Request** (was cited as `:377-384`) — ✅ **fixed 2026-08-14 (T2-1)** | **A shape the server never emits.** `requestUri` is Authlete's spelling, `expires_in` is the RFC's, and the two never appear together at the top level. `expires_in` is 600 live, not 90. The `DPoP-Nonce` header cannot appear at all (`dpopNonceRequired = False`) | `DOC_INCORRECT` / **S2** |
| `GET /api/authorize?client_id=…&request_uri=…` | `FAPI-TUTORIAL.md`, **Part 4 → Step 4: Authorize** (cited as `:390` here and re-anchored to `:391` by CUR-3c-W12; **both numbers are now stale** — T2-1 and T2-11 moved the file) — ✅ **fixed 2026-08-14** | The path is `/api/authorization` (`routes/authorization.routes.ts:7`). `/api/authorize` 404s into the SPA catch-all | `DOC_INCORRECT` / S3 |
...
|---|---|---|---|
| 9126-W1 | Accept the RFC 9126 wire format at `/api/par` | M | ✅ **DONE 2026-09-01, under plan mode.** A form-encoded body is accepted and forwarded as `parameters` **verbatim** — exact encoding and order, which the request object's signature depends on; the JSON `{parameters}` shape stays supported and every existing case still passes. `parseBasicAuth` still owns the Basic channel in both shapes. **Two things the criterion did not name.** First, the merging branches must be *skipped* on the form path: `client_id` is a real key in a parsed form body, so `appendToParams` would have sent a second copy to Authlete. Second, the controller was half the defect — `validateOrThrow(parSchema, req.body)` ran unconditionally, so the envelope schema rejected the conformant shape before the service was reached; it is now gated on `rawBody`, matching `token.controller.ts`, which validates no schema at all. **What forced it.** The deferral in `04-remediation-plan.md` §7.1 was ruled with no conformance driver. On 2026-09-01 three OpenID Foundation runs against service `2147478188` (`pg9EmudTydDs6`, `WvEz7ONBGbu7s`, `NOcwqBMX07XXa`) died at the first PAR with `CheckPAREndpointResponse201WithNoError` — after 27, 38 and 46 passing configuration checks and before a single OAuth request. **Verified live** against `us.authlete.com` service `2147478188`: form-encoded and JSON both answer `201` with `{"expires_in":300,"request_uri":"urn:…"}`; and enforcement is unchanged on the new path — no `client_assertion_type` earns `401 [A157309]`, and a valid assertion with no request object earns `400 [A363301] The request must contain a signed request object`, identical to the envelope path. `scripts/fapi2-conformance-preflight.mjs` gained a check for the wire format; the check it already had sends the JSON envelope, so it proved Authlete's enforcement while being blind to this. Scope was PAR only — see the note on the three-way grouping in `docs/agents/server-endpoints.md`. |
| 9126-W2 | Return `responseContent` as the body | S | ✅ **DONE 2026-08-14** (T1-11, under plan mode). 201 body is exactly `{"expires_in":…,"request_uri":"urn:…"}`; error bodies are Authlete's `responseContent` verbatim; `action`/`resultCode`/`resultMessage` never reach the client. Extracted as **`sendSpecBody`** (`utils/http-utils.ts`) rather than copied four times, and it keeps the envelope as a **fallback** when `responseContent` is absent — an empty body would tell the caller nothing, the same reasoning that kept `responseContent: null` visible on `/api/jar/process`. `ParSection.tsx` moved with it (five sites, not the one the audit named), plus `PAR-TUTORIAL.md` ×4 including a `python3` extractor reading `['requestUri']`, `FAPI-TUTORIAL.md`, `RAR-TUTORIAL.md` and `modules/05…/lab.md`'s `node -e` extractor. **`modules/05…/README.md` had documented the snake_case shape all along** — one doc was ahead of the code, and this made it true. |
...
| 9126-W5 | Rewrite `FAPI-TUTORIAL.md` Step 3 against a real transcript | S | ✅ **DONE 2026-08-14 (T2-1), all three clauses.** `expires_in: 90` → **600** (`pushedAuthReqDuration`); both `DPoP-Nonce` lines **removed and gated** on `dpopNonceRequired`, which is `false`, with a marker saying the nonce dance is the specification's rather than this deployment's; `/api/authorize` → `/api/authorization`, with a line recording that the wrong path returned **HTML with a 200** via the SPA catch-all instead of a 404. Two things the criterion did not name: the token response's `expires_in: 3600` (live **86400**) and its `scope: "fapi_scope openid"` — **`fapi_scope` is not a registered scope**, so it would be silently dropped, which is the same trap the file's Part 3 warns about for FAPI enforcement. The header box now splits the file explicitly: Parts 5–6 are **captured**, Parts 3–4 are **`UNVERIFIED`**. |
| 9126-W6 | State the wire-format gap in `PAR-TUTORIAL.md` and Module 05, then remove it | S | ◻ **Tutorial half DONE 2026-08-14 (T2-1); the Module 05 half stays in T2-15.** `PAR-TUTORIAL.md` now carries a two-row table separating the halves — the **request** is Authlete-shaped JSON and a conformant §2 client gets `400 Missing required body field: parameters`; the **response** has been §2.2's exact body since T1-11. **The sharper point the criterion missed:** Part 1 sells *"an SPA can call PAR from JavaScript"* as a benefit, and that is true **only because** the endpoint takes JSON — a conformant PAR endpoint requires client authentication, which is precisely what an SPA cannot do. The convenience and the non-conformance are the same fact, and the tutorial now says so instead of presenting one and hiding the other. |
