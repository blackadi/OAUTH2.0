...
> (= EH-W1). ~~**FAPI2-W3/W4/W5/W6 remain open**~~ → **all four have since shipped.** W3, W4 and W6 landed
> 2026-08-13/14; **W5 was ruled by Gate 4 the same day as DR-02** (2026-08-14 — corrected 2026-09-09, this
> box had not caught up): *qualify the claim, do not enable the profile* — taken as written, with every
> paired doc change (W3, W4, W6) already shipped by the time the ruling landed. `getStatus` reports all eight §5.3.2.1
> requirements as of FAPI2-W4 — see the work-items table below for each.
>
...
| FAPI2-W4 | Report the whole profile | S | ✅ **DONE 2026-08-14 (T1-19 batch 1).** `getStatus` adds `pkceS256Required`, `tlsClientCertificateBoundAccessTokens`, `supportedTokenAuthMethods` and the signing-algorithm lists, so the endpoint can fail honestly on all eight requirements. |
| FAPI2-W5 | Decide whether FAPI 2.0 is a claim this deployment makes | M | ✅ **RULED 2026-08-14 (DR-02) — qualify the claim; do NOT enable the profile.** The recommendation below was taken as written: teach FAPI 2.0 from the spec with the gap stated, exactly as Module 05 does for mTLS. Enabling it would have required one client with `private_key_jwt` + JWKS (**7523-W4**), PAR required, PKCE S256 required, DPoP required, rotation disabled, PS256/ES256 only — and would have broken the RFC 9700 §2.2.2 rotation lesson and the retired-grant exercises. See `05-decision-records.md` DR-02. |
