...
| FAPI2-W3 | Correct `README.md`'s feature table | S | FAPI 2.0 reads "profile not enabled; DPoP implemented and verified", with a pointer to this entry. Part of **NSSO-W4**'s single systemic fix. |
...
| FAPI2-W6 | Rewrite `FAPI-TUTORIAL.md` against real transcripts, or label them | M | Every request/response either reproducible or marked as illustrative; `/api/authorize` → `/api/authorization`; the PAR block corrected per **9126-W5**. |
