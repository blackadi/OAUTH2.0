...
| JARM-W6 | ~~Report the `form_post.jwt` anomaly upstream~~ | — | 🚫 **RETIRED 2026-08-17 — not a repo work item, by the repo owner's ruling.** Filing a support ticket with a vendor is not a deliverable of this repository: no commit can discharge it, no check can verify it, and carrying it as `OWED` meant the audit could never close on its own terms. **The technical finding is unaffected and stays** — JARM-W2 characterised it precisely (the defect is exactly one result code, `[A012305]`, where Authlete builds the `form_post` HTML body and labels it `LOCATION`), and §22.2 holds the six-row matrix. What is retired is the *obligation to tell somebody*, not the knowledge. The drafted report (`audit/JARM-W6-vendor-report.md`) is deleted with it. **Unreachable here regardless**: the trigger is the misconfiguration JARM-W1 removed on 2026-08-12. |
