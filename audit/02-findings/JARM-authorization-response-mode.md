...
|---|---|---|---|
| JARM-W1 | Set `authorizationSignAlg = ES256` on one client | S | `response_mode=jwt` returns a signed `response` JWT; Module 09a Exercise 2a gains a success transcript alongside its `[A012305]` one, and the five `UNVERIFIED` markers in that lab lose one entry. Console or `PUT /api/client/…` — no code. |
| JARM-W2 | Verify the `form_post.jwt` success path once W1 lands | S | Either the anomaly is error-path-only (as the lab carefully says) or it is general; the lab's scope caveat is replaced by the answer. |
...
| JARM-W6 | Report the `form_post.jwt` anomaly upstream | S | Filed with Authlete; the lab links the report. **No local workaround** — see F-2. |
