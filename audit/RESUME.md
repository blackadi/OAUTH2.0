...
**Next — P3 is now the head of the queue, T1-19 having closed.** T1-11's wire format (PAR/Device/DCR **and now
`/api/vci/deferred/issue`** — one batch with the SPA and lab transcripts, or the labs go stale;
`ParSection.tsx` and `DeviceSection.tsx` read the camelCase envelope fields today, and 8628-W6 already
confirmed Authlete's own `responseContent` is exactly §3.2's snake_case shape). **`buildClientInput` gained a
second reason to be looked at while doing batch 2**: it is the site of `CLIENT-UPDATE-FIELD-LOSS`, *and* the
`Client` schema's 108 properties are now known to include four the SDK does not model — see the BCL-W6 note
above before assuming the mapper can round-trip a client.
...
**Current baselines:** **1067 server tests / 72 files**, **109 client / 16**, server lint **4 pre-existing
