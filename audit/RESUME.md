...
endpoint.** Server tests **721 → 939**, files **63 → 69**, in six new integration files:
...
**One new defect came out of it, and it is the same shape as `/api/jar/process`.**
**`POST /api/vci/deferred/issue` authenticates nobody** — it never collects an access token, and the Authlete
API it calls (`VciDeferredIssueRequest`) has no `accessToken` field to validate one. Authlete splits the flow:
`/vci/deferred/parse` is where the token is checked, and this server never calls it. Both `AGENTS.md` and
...
plan mode. Full evidence in `AGENTS.md` under the VCI bullet's *Known gap*, and a characterization block in
`tests/integration/vci.routes.test.ts` that names the fix. **This is the next code item.**
...
**Next, in descending value:** the **VCI deferred-issue gap** above (plan mode); **T1-11's three spec-shaped
endpoints** (PAR/Device/DCR return Authlete's camelCase envelope instead of the RFC body — probe-confirmed,
but it breaks `ParSection.tsx:112` and `DeviceSection.tsx:159-160`, so server + SPA + lab transcripts belong
in **one** batch); **T1-19**'s remaining small items; then **Tier 2** documentation and **Tier 3** decisions.
...
| `03-curriculum-audit.md` citing `PROGRESS.md:1401` for the RFC 8446 verification claim | Wrong **when written** — it matched no revision of the file. Correct target `PROGRESS.md:2282-2283` | fixed 2026-08-11 during T0-6; see `04-remediation-plan.md` §6.3 |
