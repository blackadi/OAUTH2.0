...
**Status: ✅ RULED 2026-08-14 — decline both, as one decision.** Taken as written. The grounds hold and the third has strengthened: `prompt=none` now answers *"is the user still logged in?"* correctly, because T1-7 landed — this record asked for that to come first, and it did. **The three documentation consequences (FCL-W2, FCL-W3, SM-W2) were discharged the same day under T2-5** — see below; this sentence originally said they "remain… still open," which contradicted this entry's own §T2-5 note 21 lines down and was corrected 2026-09-09. The revisit trigger is unchanged and is the reason the record covers two specifications: **durable OP session identity has four consumers** — Session Management, Front-Channel Logout, back-channel logout's `sid` mode and Native SSO (DR-04) — and building it for any one reopens all four.
