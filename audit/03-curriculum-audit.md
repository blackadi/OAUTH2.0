...
### F-4 — two foundational dates are carried, not verified (S4) — ✅ **CLOSED 2026-09-08 (CUR-3a-W4)**

> **Status: closed.** Both dates fetched and confirmed exact: RFC 8446 is August 2018 and RFC 9110 is STD 97,
> June 2022. `modules/00…/README.md:87-88` was also updated separately (T2-4, 2026-08-14) to lead with
> RFC 9846 (the current TLS 1.3 number) rather than the superseded RFC 8446. See `RESUME.md` §2.3.
...
| CUR-3a-W2 | Fix `ParSection.tsx:38` → `:43` | S | ✅ **DONE 2026-08-14 (T2-10) — as `:41`, not `:43`.** The `sessionStorage.setItem('pkce_code_verifier', …)` **write** is at `:41` and the `getItem` **read** at `:34`; the criterion's `:43`/`:36` were correct in early August and the component has since shifted by two lines. Module 03's README now names both, and says which is which. **Second item in this bundle whose *correction* had itself gone stale** — see CUR-3b-W7. 3a-F6. |
...
> ### ✅ CLOSED 2026-08-14 (T2-14) — **the curriculum was right and this audit's entry was incomplete**
>
> Fetched `https://www.rfc-editor.org/rfc/rfc9101.html`. **§10.1 is titled *"Choice of Algorithms"***, and its
> text is verbatim the sentence Module 05 attributes to it: *"When sending the Authorization Request Object
> through the `request` parameter, it MUST be either signed using JWS or signed and then encrypted using JWS and
> JWE, respectively, with algorithms considered appropriate at the time."* **§10.8 exists and is titled
> *"Cross-JWT Confusion"***, so claim 9 is confirmed too.
>
> **The Phase 2 entry mapped the signing requirement to §6.2, which is *"JWS-Signed Request Object"* — a
> validation rule for a server receiving one.** Both sections are real and they answer different questions;
> Module 05 cites the one that carries the sentence it quotes. **Sixth Phase 3 finding that corrects the audit
> rather than the curriculum**, and the pattern holds: it arose where the audit reasoned from its own earlier
> fetch list while the curriculum had read the document.
...
### 3b-F5 — Module 05 names the two DPoP call sites that still build `htu` wrongly, and does not say so (S3, = 9449-W1) — ✅ **RETIRED 2026-08-13 (9449-W1)**

> **Status: closed by fix, not by doc update.** All five DPoP call sites — including `token.service.ts` and
> `par.service.ts` — now route through `dpopHttpTarget()` (`utils/dpop.ts:157-161`), which excludes the query
> string per RFC 9449 §4.2. The defect this finding describes no longer exists in the code; Module 05's code
> map should be updated to describe the fix rather than the (now historical) gap. See `RFC9449-dpop.md`
> 9449-W1.
...
| CUR-3b-W2 | **Add the grep step to the remediation checklist** | S | ✅ **DONE 2026-09-09.** `docs/agents/curriculum-contract.md`'s *"grep the curriculum for the symptom you changed"* rule now adds: search for the **phrase** naming the defect, not only the error string, and check every module whose Prerequisites/forward-references name the one you edited. Names Module 08→Module 10 as the worked example. 3b-F9. |
...
| CUR-3b-W6 | Fix Module 06's two handler references | S | ✅ **DONE 2026-08-14 (T2-10), in the same commit as 8693-W3 as instructed.** Both refs corrected to `:47-52` and `:32`, and *"read those six lines"* is now *"read the create-request literal"* — the criterion's exact wording, and it matters because *six lines* counted the ⚠️ comment the old number pointed at. The lab also gains a short note on **why** the anchor is the comment rather than the number, since this reference has now drifted twice. 3b-F8. |
| CUR-3b-W7 | Fix `introspection.controller.ts:47` in Module 09a and `AGENTS.md` | S | ✅ **DONE 2026-08-14 (T2-10) — and this criterion's own replacement numbers were stale when applied.** It says the RFC 9470 branch is at `:81-97` and `parseBearerError` at `:20-36`. As of 2026-08-14 the branch is **`case "FORBIDDEN"` at `:142-167`** and `parseBearerError` is at **`:45`** — the file grew (T1-1's auth gate, the DPoP-nonce relay). Fixed in `AGENTS.md`, `modules/09a…/lab.md` **and** `docs/STEP-UP-AUTH-TUTORIAL.md`'s appendix, which the item did not name and which read `:114` — the *validation-error* branch. **All three now name the branch as well as the line**, so the next growth of that file leaves them findable. 3b-F10 class. |
| CUR-3b-W8 | Say that this deployment's step-up response is a 403 | S | ✅ **DONE 2026-08-14 (T2-11).** The *"❌ 403 for a step-up requirement"* entry now names `server/src/controllers/introspection.controller.ts` as an instance **and draws the distinction rather than just conceding it**: the 403 is the AS → RS introspection response, §3's challenge is the RS → client response, this repo implements only the first, *"you write the second."* Cited without a line number on purpose — 3b-F10's own `:84-96` had already drifted to `:142-167`, and this is the fourth item in the audit to be re-anchored by content. Closes 3b-F10 with the sentence it proposed, plus the reason `curl` returning 403 does **not** contradict the ❌. |
| CUR-3b-W9 | Add the signing-algorithm `shall` to Module 10 | S | ✅ **DONE 2026-08-14 (T2-12) — with the verdict corrected from FAIL to PARTIAL.** The criterion's **FAIL** was already wrong when written (§2.2 had recorded `ES256` as advertised) and became more wrong on 2026-08-12, when T1-2's single RSA key added **`PS256`** — so the live `id_token_signing_alg_values_supported` carries *both* algorithms the profile permits, plus three HMAC ones. **What decides the row is `idTokenSignAlg`, which is per client**: `ES256` on three, **`HS256` on `1523514379`**, the client these labs use. Both documents now carry the row with that four-client table, and `lab.md` cross-links Module 05's HS256 observation as instructed. Row 7 is **NOT REACHABLE**. **The finding underneath is better than the row**: a conformance report built from discovery metadata alone scores this **PASS**, so `lab.md`'s Exercise 7 now separates PARTIAL / NOT REACHABLE / NOT EVIDENCED as three different reporting errors, and states the general test — *which principal does this `shall` bind?* 3b-F14. |
...
| CUR-3b-W11 | Say which DPoP call sites still send a query string in `htu` | S | ✅ **RETIRED 2026-08-13 — 9449-W1 landed.** All five call sites (including `token.service.ts` and `par.service.ts`) now route through `dpopHttpTarget()`; there is no longer a call site to name. Module 05's code map should drop the "still builds `htu` with the query" claim (3b-F5) rather than update it. |
| CUR-3b-W12 | Verify RFC 9101 §10.1 | S | ✅ **DONE 2026-08-14 (T2-14) — and it resolved in the curriculum's favour.** §10.1 is **"Choice of Algorithms"** and carries verbatim the MUST-be-signed sentence Module 05 attributes to it; §10.8 is **"Cross-JWT Confusion"** and exists, settling claim 9. The Phase 2 entry's mapping to §6.2 (*"JWS-Signed Request Object"*) was the incomplete one — both sections are real and answer different questions. **The same fetch closed CUR-3b-W3 as a by-product**, which the item did not anticipate: the *precedence* rule is **§6.3**, not §5, the phrase occurs **exactly once** in the document, and the repo's quotation carried an extra word (*"the parameters **included** in the Request Object"* for *"the parameters in the Request Object"*). **Two of the audit's last three fetches, and both went against the audit.** |
| CUR-3b-W13 | Cite the Native SSO header date alongside the approval date | S | ✅ **DONE 2026-08-14 (T2-14).** Both dates now appear together — **text dated 16 Jan 2025, approved as the 2nd Implementer's Draft 2025-10-17** — in `SPEC-INVENTORY.md`'s row and in Module 09a's status caveat and comparison table. The caveat also states **why** both are needed, which is the part that generalises: *the header date tells you which text you read, the approval date tells you what standing it has*, and citing one leaves a reader unable to tell whether they have the revision you meant. `SPEC-INVENTORY.md` already carried the rule generally; the module now points at it instead of restating half of it. 3b-F12. |
...
| CUR-3b-W15 | Correct the archetype count in the audit's own entry | S | ✅ **DONE 2026-08-14.** `FAPI-2.0-ATTACKER-MODEL.md` now says **six** in its thinking block and sources list, matching its own normative table. 3b-F13. |
| CUR-3b-W16 | Name RFC 9440 in Module 05's mTLS revisit conditions | S | ✅ **DONE 2026-08-14 (T2-14).** The revisit trigger now names **RFC 9440, *Client-Cert HTTP Header Field* (Informational, July 2023)** and its `Client-Cert` / `Client-Cert-Chain` fields, with the advice to cite the RFC rather than a vendor header name — the ad-hoc ones it lists (`x-amzn-mtls-clientcert`, `X-Client-Cert`, `X-SSL-Client-Cert`) all predate it and none interoperates. **Two things added beyond the criterion, and they are what make the citation useful rather than decorative:** what Informational status *means here* — it standardises the header, not any obligation to trust it — and §4's insistence that an origin accept it only from a proxy it authenticates, since a header is trivially forgeable by anyone who can reach the origin directly. **That caveat is precisely why this is a revisit trigger and not a plan.** 3b-F6. |
...
|---|---|---|
| **Module 09b Ex 7** — four transcripts, two observations drawn from them, two `UNVERIFIED` markers, the module README's status row and a `SPEC-INVENTORY.md` row | **DR-03** (`verifiableCredentialsEnabled: true`), then **VCI-W6** (`credentialJwks`) broke the *rebuilt* version two days later | ⚠️ **was live drift for two commits** → ✅ **rebuilt 2026-08-14 from fresh probes.** The exercise is now *better*: the old lesson was "every VCI endpoint refuses because the feature is off"; the new one is **"enabling a feature is not the same as configuring it"** — `/vci/metadata` returns a conformant §12.2.4 document while `/vci/jwks` and `/vci/jwtissuer` fail `A403201`/`A417202` for want of a credential-issuer JWK Set, and the result codes changed from `NOT_FOUND` to `INTERNAL_SERVER_ERROR`, which is the honest transition. **Then VCI-W6 closed on 2026-08-14 and broke the rebuild** — so it was rebuilt *again*, into **three dated states** (`NOT_FOUND` → `INTERNAL_SERVER_ERROR` → `200`), each transition naming a different missing value, plus a check that `/vci/jwks` publishes no private key member. **Third consecutive rebuild that improved the lab**, and the first where closing a gap improved a *diagnosis*: the issuance marker had been blaming the missing key, which hid the fact that nothing else was missing — it now needs only a wallet this repo does not contain. **The register's own lesson generalises: a lab rebuilt around a gap inherits that gap's lifetime**, so a fix-driven rebuild should be written to survive the gap closing |
...
| ID | Item | Effort | Acceptance criteria |
|---|---|---|---|
| CUR-3c-W1 | **Adopt the curriculum's `UNVERIFIED` convention across the nine tutorials** | M | ✅ **DONE 2026-08-14 (T2-1).** **5 markers in 3 files → 29 in 9.** The convention is defined **once**, in `docs/README.md`, as three labels — **captured** / *illustrative* / **`UNVERIFIED`** — and each tutorial carries only its own file-specific facts, so nine near-duplicate boxes cannot drift apart. None of the three words is invented: `UNVERIFIED` is the curriculum's, *illustrative* is FAPI2-W6's own acceptance criterion, and **captured** is `TOKEN-EXCHANGE-TUTORIAL.md`'s existing *"what this server actually returns (captured 2026-08-06)"*. **That file is the model the other eight were brought up to** — it already had the convention and batch 3c graded it exemplary, which means this was never *"three files away"* in the curriculum: there was an in-tutorial precedent nobody had generalised. **The premise had to be re-probed before a word was written, and this is the part to carry: batch 3c's reproducibility table was stale in four places, all in the *runnable* direction.** T1-6 registered `payment_initiation`, so RAR's three "cannot have been produced" transcripts are now derived from a real 2026-08-12 round trip; T1-6 also set `bcDeliveryMode = POLL` on `1523514379`, so CIBA's flow runs — **on that one client, and body credentials 401 against it because it is `CLIENT_SECRET_BASIC`, which is the configuration the tutorial itself recommends**; T1-3 created a `private_key_jwt` client with a JWKS, so half of FAPI Part 4 is runnable; DR-05 enabled CIMD. Only **Native SSO** is still wholly unrunnable (`nativeSsoSupported = false`). **Writing "unreproducible" from the 3c table would have been wrong in four files.** Six live literals were corrected on the way — five `expires_in: 3600` where `accessTokenDuration` is **86400**, and PAR's `expires_in: 90` where `pushedAuthReqDuration` is **600** — plus a `{"action":"CREATED","request_uri":…}` block in `RAR-TUTORIAL.md` that was **half Authlete's envelope and half the RFC's body, a shape that never existed** (a T1-11 residue), and an introspection block showing `authorization_details` at the top level where the live response is `authorizationDetails.elements[]` with the RFC's common data fields flattened into an **`otherFields` string**. 3c-F7. |
| CUR-3c-W2 | **Correct `MCP-OAUTH-TUTORIAL.md`'s opening sentence** | S | ✅ **DONE, verified 2026-08-14.** The file's precondition table now shows **both** conditions **met** — self-consistent issuer (DR-11) and CIMD (`clientIdMetadataDocumentSupported`, CIMD-W3) — a reversal of the original premise rather than a wording fix, since both preconditions were unmet when this item was written and both were closed by later decision records. = **8414-W5** / **CIMD-W2**. |
...
| CUR-3c-W6 | Fix the step-up challenge status everywhere | S | ✅ **DONE 2026-08-14 (T2-11).** All three: the Part 1 diagram arrow is `401` with a paragraph under it saying so was the single most important number in the file, and both Part 5 transcripts are re-framed as **Response 1** (the legitimate 403) beside a new **Response 2** (§3's 401). **"Everywhere" was underspecified and the widening was right:** a **fifth** location existed in `docs/DATA-FLOWS.md`, outside the nine tutorials and outside the modules, drawing `RS-->>C: 403 Forbidden` — the identical defect in a document nobody had checked for it. 3c-F6. |
| CUR-3c-W7 | Fix the two wrong authorization paths | S | ✅ **DONE 2026-08-14, with T2-1** — same files, same pass, so splitting it would have meant editing both twice. `FAPI-TUTORIAL.md`'s Step 4 and `PAR-TUTORIAL.md`'s Step 2 diagram now read `/api/authorization`, each with a line saying what the wrong path *did*: matched no route, fell through to the SPA catch-all, and returned **HTML with a 200** — which is why neither was ever reported as an error. **The remaining `/authorize` occurrences are deliberate and were left**: mermaid arrows and RFC-generic illustrations, where the abstract endpoint name is the point. `check-docs.mjs` cannot see any of them either way — it validates `/api/…` paths, and a bare `/authorize` has no prefix to match. 3c-F5, = 9126-W5. |
| CUR-3c-W8 | Narrow Module 06's warning about Part 7 | S | ✅ **DONE.** `modules/06…/README.md:535-544` now says Part 7 "is the part you can trust", with a dated note explaining the warning used to say the opposite and was corrected. 3c-F8. |
| CUR-3c-W9 | Add the vendor-documentation finding to Part 12 | S | ✅ **DONE.** `TOKEN-EXCHANGE-TUTORIAL.md:711-732` now states it directly: Authlete's documentation says to return `issued_token_type` and Authlete does not supply it; the AS must synthesize it. 3c-F9. |
| CUR-3c-W10 | Fix `TOKEN-EXCHANGE-TUTORIAL.md`'s three stale handler references | S | ✅ **DONE.** `:664` and `:821` now cite `:47-52` directly; `:821` adds a note distinguishing it from the ⚠️ comment above it that older revisions cited. Folded into **8693-W3**. |
...
### 3d-F3 — the TLS row asserts an obsoletion this audit has not verified, and Module 00 contradicts it (S3) — ✅ **CLOSED 2026-08-14 (T2-4) / re-confirmed 2026-09-08 (CUR-3a-W4, widened per CUR-3d-W1)**

> **Status: closed.** Both RFCs fetched: RFC 8446 (Aug 2018) is obsoleted by **RFC 9846** (`rfc8446bis`, Jul
> 2026, same wire version, backward compatible), and RFC 9110 is confirmed STD 97 / Jun 2022. Module 00's
> `README.md:87-88,99-103,268` now cites RFC 9846 as primary, keeps RFC 8446 named for readers who know it by
> that number, and explains the obsoletion in a dedicated callout — so `SPEC-INVENTORY.md` and Module 00 agree.
> The RFC 9864 caveat (`README.md:109-113`) is fetched and confirmed too: the `ES256` deprecation is COSE-only
> and does not affect this curriculum. No open question remains in this row.
