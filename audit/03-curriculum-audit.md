...
absent on this service (probe 1 §3.4), and `PROGRESS.md:983-984` records the live success transcript. Neither
...
| 11 | Both FAPI endpoints return **HTTP 200 with an error body and a stack trace**; *"the deployment cannot report its own FAPI posture"* | `README.md:437`, `lab.md:229-246,475` | ✅ = `FAPI-2.0-SECURITY-PROFILE.md` F-1 and `ERRORHANDLER-…`; `AGENTS.md`'s claim is confirmed | **Accurate — exemplary** |
...
| 16 | Exercise 1's set-piece: *"which attacker does this repo's logout open redirect serve?"* → **A1** | `lab.md:81-91`, `:469` | ❌ **stale** — same cause | `DOC_INCORRECT` / S3 — 3b-F9 |
| 17 | Conformance summary row *"No open redirectors \| **FAIL** — logout endpoint (Module 08)"*, and remediation item 1 *"The open redirect… a one-line fix"* | `lab.md:423,430-431` | ❌ **stale** — same cause; and the fix was not one line | `DOC_INCORRECT` / S3 — 3b-F9 |
| 18 | Remediation order: open redirect → `fapiModes` → DPoP → token lifetime → 600→599 → FAPI endpoints, *"the reasoning matters more than the list"* | `lab.md:428-444` | ✅ **independently reaches RESUME §5.4's conclusion** — one `fapiModes` change subsumes most FAIL rows, and observability is not last | **Accurate — exemplary** |
...
|---|---|
| `OIDC-RP-INITIATED-LOGOUT-1.0.md` — open redirect | `logout.service.ts:33-63` — `isAllowedPostLogoutRedirectUri()` now parses with `new URL()` and compares **origins exactly**; unparseable or non-http(s) fails closed |
...
`lab.md:81-91` (an Exercise 1 set-piece with a `<details>` answer), `lab.md:423` (the conformance table),
`lab.md:430-431` (remediation item 1), `lab.md:469` (the verification block). A learner doing Module 10 today
...
| ID | Item | Effort | Acceptance criteria |
|---|---|---|---|
| CUR-3b-W1 | **Re-point Module 10 at the fixed logout endpoint** | S | All five references (`README.md:205`; `lab.md:81-91,423,430-431,469`) reflect the 2026-08-10 fix, in Module 08's style: what it used to do, why it passed, what it does now. Exercise 1's A1 question survives as history — it is still the right answer about the *old* endpoint and still the best illustration of the weakest-attacker point. 3b-F9. **Highest-value item in the batch.** |
...
| CUR-3b-W5 | **Teach `check-docs.mjs` the prose reference form** | S | A reference of the form *"Line ~89"* following a `**path**` span is validated against the file's length. Extends **CUR-3a-W1**; together they close the two forms that let two EOF-overrunning pointers through. 3b-F4. |
...
| CUR-3c-W3 | **Enforce the trailing tilde in `sd-jwt.mjs`** | S | An input with no `~` is rejected; a non-empty final element is treated as a KB-JWT only if it parses as a three-part JWS; `7.1/1` becomes a real check that can FAIL. A regression case: a four-Disclosure credential with the tilde stripped must be REJECTED, not accepted with three. 3c-F1. |
| CUR-3c-W4 | Fix `present --out` and `issue --iss` | S | `--out` honoured on both presentation paths; `--iss` either required on `issue` or explained at `7.1/2c` as the Verifier's policy. 3c-F2, 3c-F3. |
| CUR-3c-W5 | **Give `sd-jwt.mjs` a test file** | M | The `scripts/` directory is outside both Vitest configs, so the repo's only SD-JWT implementation has no regression net. Minimum: the §4.2.3 published vector, salt length and uniqueness, the five attacks Module 09b runs, and CUR-3c-W3's malformed-input case. Would have caught 3c-F1. |
...
| CUR-3c-W11 | Extend `check-docs.mjs` to endpoint paths | M | A path in a fenced block matching `/api/[a-z-]+` is checked against the routes actually mounted in `app.ts`. Catches 3c-F5's class, which is the one that costs a reader the most. Third extension proposed, after **CUR-3a-W1** (bare paths) and **CUR-3b-W5** (prose refs) — Phase 4 should scope the three together. |
...
3. **The curriculum does not follow the instruction.** `modules/00…/README.md:87` and `:250` still cite RFC
   8446, and `PROGRESS.md:1327-1328` records RFC 8446 as *"verified against primary sources"*. So `SPEC-INVENTORY.md`
...
(**RPL-W1**), and reality disagreed with both: the "one-line fix" framing appears in `PROGRESS.md:495-510`, in
