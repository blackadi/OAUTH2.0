...
| 8 | [`RFC9470-step-up-authentication.md`](02-findings/RFC9470-step-up-authentication.md) | ~~latent S1~~ → **S3** | ✅ **RETIRED 2026-08-12 (T1-7).** The fabrication block is deleted; `utils/step-up.ts` answers absence as "no", and OIDC-W1 shipped with 9470-W3 as one change so the route was built correctly rather than built at all. Pre-fix description and full history: `02-findings/RFC9470-step-up-authentication.md` F-3 | ✅ its own header reads **S2 + latent S1**, not S1 |
...
| 22 | **9126-W5** = CUR-3c-W7 ⊂ FAPI2-W6 | ✅ **shipped 2026-08-14 inside T2-1** — same two files, same pass. **FAPI2-W6 appears here and in no tier row**, which is how an ID can be counted as covered without being scheduled; T2-1 discharges it |
...
| **T1-14** | ✅ **SHIPPED 2026-08-13.** **`jwt.verify` hygiene rule** — `issuer` + `audience` on every call, recorded in `AGENTS.md` | JOSE-W1 ✅ ⊃ BCL-W1 ✅, BCL-W3 ✅, BCL-W7 ✅ | 📋 Planned under plan mode, with T1-15. **The rule has two clauses and the second is the one that gets skipped**: pass both options, *and* refuse when the expected values are unconfigured — omitting an option because its value is empty silently downgrades the check to *any issuer, any audience*, and looks identical in code and logs. Two new settings carry the expectations (`BACKCHANNEL_LOGOUT_ISSUER`/`_AUDIENCE`), deliberately **not** `JWT_ISSUER`, since on that endpoint this server is the **RP**. §2.6 went from **5 of 11 steps to 11 of 11**, each rejection driven live against a locally-served JWKS. **BCL-W3 and BCL-W7 rode along** (same function): misconfiguration is now 500 and is checked *before the token is read* — a server that cannot verify a signature must not render any verdict on the token, not even a true one — and `Cache-Control: no-store` is set before any branch. **The endpoint had zero unit/integration coverage**; it now has 16 tests |
| **T1-15** | ✅ **SHIPPED 2026-08-13.** **Terminate the user's session on back-channel logout**, not the request's | BCL-W2 ✅ | 📋 Planned under plan mode. `req.session` is the **caller's** session — another OP's server, posting server-to-server with no browser cookie — so the endpoint destroyed nothing, answered 200, and the sending OP believed the user was logged out. `destroySessionsForSubject` (`utils/session-store.ts`) now looks sessions up by `sub`. **The finding the work item did not predict:** the two supported stores return **different shapes** from `Store.all()` — MemoryStore an object keyed by session id, connect-redis an array with `sess.id` — so a handler for one silently terminates nothing against the other, which is the very failure being fixed; a test drives the **real** MemoryStore. `AGENTS.md`'s *"properly destroys `req.session`"* is corrected: it did exactly that, and that was the bug. **Verification split honestly** — receipt and the store wiring proven live (`destroyed: 0`, not `null`), a logged-in session dying proven by the real-store test, delivery still unexercised because no client registers a `backchannel_logout_uri`. Suite 684 → **709** / 61 files |
| **T1-16** | ⚠️ **SHIPPED 2026-08-13 — one line in, and it does *not* unblock FED-W2.** **`{ requestBody: {} }` to `federation.configuration`** | FED-W1 ⚠️, FED-W2 ⛔, FED-W5 ✅ | **The fifth work item this phase whose acceptance criteria name an unreachable outcome**, caught by probing before writing code. Omitting `requestBody` makes the SDK send no `Content-Type`, so Authlete answers `400 [A258201]` and the SDK throws — the caller got a 400 for our fault. With the body present the call reaches Authlete's *real* answer, `INTERNAL_SERVER_ERROR` / `[A316201] Because a JWK Set for federation has not been set up…`. So the fix **changes the failure rather than removing it**: both routes now answer **500 naming the missing configuration**, verified live. **FED-W5 closes with it** — the controller's mapping was already correct and had only ever seen a thrown SDK error. **FED-W2 stays blocked** on a federation JWK Set, which is feature enablement (Theme 2 / Tier 3), not a fix. `federation.service.ts` had no tests and *could not* have had any: `tests/helpers/mock-authlete.ts` had no `federation` member despite `AGENTS.md` calling it complete. Both fixed |
...
| **T1-18** | ✅ **SHIPPED 2026-08-13, across two commits.** **Server-misconfiguration vs bad-token statuses** — unset `JWKS_URI` → 500, not 400 | BCL-W3 ✅, FED-W5 ✅ | Same pattern, **different fixes**, which is the part worth knowing. **BCL-W3** needed real work: the throw happened in *our* code, so the catch-all had to learn to tell a sender's fault from ours — and the config check was moved *before* the token is parsed, because a server that cannot verify a signature must not render any verdict on the token, not even a true one. **FED-W5** needed none: once FED-W1 stopped the SDK throwing, Authlete's own `INTERNAL_SERVER_ERROR` reached a controller mapping that was already right. Both verified live |
| **T1-19** | ✅ **COMPLETE 2026-08-14.** **Remaining contained code items**: `normalizeGrantType` total; echoing `default` branches; `attributes` validated; duplicate client-listing `fetch()`; `Cache-Control: no-store` on the logout receiver; `parseBearerError` quoted-string aware; PRM `bearer_methods_supported`; PRM well-known path; 405 on `/api/par`; JAR built from named fields; dev JWT made §2-shaped; `computeFapiMode` total over `fapiModes`; `/api/fapi/status` reports the whole profile | B1-W3 ✅, B1-W4 ✅(=7009-W3), ATTR-W1 ✅, BCL-W6 ✅, BCL-W7 ✅, 9470-W6 ✅, 9728-W2 ✅, 9728-W1 ✅, 9126-W3 ✅, 9101-W5 ✅, 9068-W2 ✅, FAPI1-W2 ✅, FAPI2-W4 ✅ ⊃ 7636-W2 | Independent of each other; batch freely. **All 13 shipped 2026-08-14, in three batches.** **Batch 1** (9728-W1/W2, 9126-W3, 9470-W6, B1-W4, FAPI2-W4): *three of those were one defect — an endpoint answering **200 with HTML** where it meant "no"*, and FAPI2-W4 completed what FAPI2-W1 started, taking `/api/fapi/status` from six of eight FAPI requirements to seven, with the eighth recorded as unreportable rather than left absent. **Batch 2** (FAPI1-W2, ATTR-W1, BCL-W6): BCL-W6 corrected an `AGENTS.md` generalisation about the SDK that would otherwise have made its own fix silently deliver logout tokens to nobody — `Client$inboundSchema` *collects* unmodelled members where `Service$inboundSchema` strips them. **Batch 3, under plan mode** (B1-W3, 9101-W5, 9068-W2): all three were *a value the caller controls, or a value nobody supplied, becoming a server assertion.* Two criteria under-counted their own defect — B1-W3's named three map additions and missed that **`CIBA` had no entry at all**, and 9068-W2's ordering note exempted itself from plan mode by judging the *file* (`createLocalJWT.ts`, not listed) rather than the call chain, which runs through `token.operations.service.ts` **and** `token.management.controller.ts`, both under **Token issuance**. **The trigger is the concern, and the concern travels with the parameter.** 9101-W5 also removed an unnamed `req.query` mutation, and 9068-W2 turned up two query parameters `openapi.routes.ts` advertised and the controller dropped. |
| **T1-20** | ✅ **SHIPPED 2026-08-13.** **Use `parseBasicAuth` for CIBA client authentication** — the same three-channel logic as `par.service.ts:39-54`, with a unit test per channel | CIBA-W3 ✅ | 📋 Planned under plan mode. **`AGENTS.md`'s recommended CIBA configuration now actually works** — the guide and the code had disagreed since the endpoint was written. Verified live both ways: Basic reaches `USER_IDENTIFICATION`; body credentials for that `CLIENT_SECRET_BASIC` client now earn `401 [A157357]` instead of being **silently converted** onto the Basic channel, which is the guessing `par.service.ts` forbids. `appendToParams` extracted to `utils/params.ts` and shared. The SPA gained a Client Auth Method selector. **A CI gap surfaced while verifying**: the client job ran only `vite build`, which does not typecheck, so `npm run typecheck` was never invoked and 16 client test files never ran — 4 real type errors had accumulated. Fixed, and both gates added. Suite 713 → **721** server / **109** client. Independent of CIBA-W1/W2, which are deferred (§7.1) |
...
| # | Action | Items | Notes |
|---|---|---|---|
| **T2-1** | ✅ **DONE 2026-08-14.**  **Adopt the `UNVERIFIED` convention across the nine tutorials** | CUR-3c-W1 ⊃ **9396-W4** ⊃ **FAPI2-W6** ⊃ 9126-W5 = CUR-3c-W7; and the tutorial halves of **9126-W6** and **CIBA-W5** | **The single highest-leverage item in the plan, and the prediction held.** 5 markers in 3 files → **29 in 9**; the three labels (**captured** / *illustrative* / **`UNVERIFIED`**) are defined **once** in `docs/README.md` so nine copies cannot drift, and all three words were already in use in the repo rather than coined here. **9396-W4 was the worked example and it inverted**: T1-6 registered `payment_initiation`, so RAR's three "unproducible" transcripts are now taken from a real 2026-08-12 round trip. **Four of batch 3c's reproducibility verdicts were stale in the runnable direction** — RAR, CIBA, FAPI and MCP — so *"probe before writing"* was the load-bearing rule, not the writing. Only Native SSO is still wholly unrunnable. Six stale literals fixed, one impossible hybrid response shape removed, and `docs/README.md` gained the two index rows STEP-UP and MCP were missing (part of CUR-3c-W14). **Coverage note:** FAPI2-W6 had **no tier row of its own** — it was reachable only through cluster 22 in §5.2 — so the plan's mechanical coverage check counted it without scheduling it. It is discharged here because its criterion *is* T2-1's for one file |
| **T2-2** | ✅ **DONE 2026-08-14.**  **The seven stale open-redirect references** | CUR-3b-W1 ⊃ CUR-3d-W2 | §6.3. Module 10 ×5 + `final-exam-answers.md:227-229` + **`exams/final-exam.md:76`**, found 2026-08-12 during T0-3's phrase grep — the *question* still asserts the `startsWith` check, so it and its answer key had drifted apart. Six → **seven** |
| **T2-3** | ✅ **DONE 2026-08-14** — and extended: the rule now covers **configuration** changes, which have no symptom string to grep for.  **Fix the remediation grep rule** — search the *phrase*, and check every module cross-referencing the one edited | CUR-3b-W2 | The rule that would have prevented T2-2 |
| **T2-4** | ✅ **DONE 2026-08-14.**  **The TLS citation reconciliation** | CUR-3d-W1's outcome, CUR-3a-W4 | §2.1's four fetches applied. Module 00's two TLS citations now lead with **RFC 9846 (Jul 2026)** and name RFC 8446 as the superseded number a reader may recognise; RFC 9110 is sharpened to **Internet Standard, STD 97**. A note under the table teaches the distinction rather than just making the edit — *the wire protocol did not change, so a document citing RFC 8446 is not wrong about anything except which number to cite*, which is the general point about obsolescence living in Datatracker metadata. **`SPEC-INVENTORY.md`'s TLS rows were left untouched, as instructed — but its RFC 9864 annotation was wrong twice** and this item found it: the date read **Oct 2025** against §2.1's verified **Dec 2025**, and the scope read *updates RFC 7518* where the header block says **7518, 8037 and 9053**. RFC 9864 now has its own §1 row. **A date carried from recall, in the file whose job is citation provenance** — the defect that file exists to prevent, and the reason T2-5's per-row "URL fetched + header line read" discipline is worth the effort |
| **T2-5** | ✅ **DONE 2026-08-14.** **One `SPEC-INVENTORY.md` pass** — 15 IDs in one commit, each row carrying its fetched URL and header line | cluster 29 + **FED-W4** | Theme 5's remedy. **The coverage sweep came first and paid for itself: 5 of the 15 were already shipped** (HSK-W2, PS-W1, §2.1's outcome, NSSO-W3, ATTR-W3 — the last two done but never *marked*, so they read as open). **A sixteenth ID was missing from the cluster list** — **FCL-W3**, which DR-08 owed here and nothing else would have caught. Ten fetches, each recorded with its URL and header line in a new **Provenance** section. **Four criteria were stale, in four different ways:** HAIP-W2 and MDL-W3 do not target `SPEC-INVENTORY.md` at all (their rows are in `01-spec-matrix.md` §3, and neither subject appears anywhere in `docs/`); ATT-W1's *"advertised but unconfigured"* was falsified by T1-5/DR-07 **withdrawing** the method; FED-W3's *"unless it can be sourced"* resolved to **sourced — and the version was never the problem**. Three rows were **confirmed rather than corrected** (RFC 9901, OID4VP, Federation's dates), which is recorded, because a pass that reports only errors cannot distinguish *checked and correct* from *never checked* |
| **T2-6** | ✅ **DONE 2026-08-14** (CUR-3c-W3 + W4 + W5).  **`sd-jwt.mjs`: enforce the trailing tilde, fix `--out`/`--iss`, add a test file** | CUR-3c-W3, CUR-3c-W4, CUR-3c-W5 | The wrong ACCEPT is the substantive one. `scripts/` is outside both Vitest configs — the repo's only SD-JWT implementation has no regression net |
| **T2-7** | ✅ **DONE 2026-08-14** — four forms, 103 → ~1,400 references validated, three real defects found.  **Teach `check-docs.mjs` four reference forms** — bare paths, prose `Line ~NN`, `/api/…` endpoint paths in fenced blocks, and **`*.md:NNN` markdown line refs** | CUR-3a-W1 + CUR-3b-W5 + CUR-3c-W11 + **the fourth gap found 2026-08-11** | **Scope as one change.** Two EOF-overrunning pointers slipped through the first two gaps. The fourth gap let **20 `PROGRESS.md` citations drift silently** (§6.3) — the checker validates only `server/`\|`client/` source refs, so no markdown-to-markdown pointer is checked at all. Note the checker can only catch *past-EOF* refs; the `PROGRESS.md` drift was mostly *in-range and wrong*, which argues for §6.3's option (b) — cite `PROGRESS.md` by **section heading** — as the real fix, with the checker as the backstop |
| **T2-8** | ✅ **DONE 2026-08-14.**  **The claimed-working/flag-off table**, derived from live configuration | NSSO-W4 ⊃ FAPI2-W3, MCP-W3, VCI-W1's doc half | **Theme 2 had drifted in *both* directions by the time this was written, and the second direction was invisible.** Two of the four features are now **on** — VCI (DR-03 + VCI-W6) and CIMD (DR-05) — so the defect was no longer *"claimed working, flag off"* but **"working, and absent from the feature tables entirely"**: `README.md` had no VCI row and no MCP row. Both added, with honest qualifications. **The deliverable is the derivation, not the rows**: a note names the five service fields the statuses depend on, gives the read-only command that prints them, and records the captured values with a date. The command was **run before publishing** and prints exactly the values the prose quotes; it prints `<set>` rather than `credentialJwks` itself, because that field holds a private scalar, and uses `json.dumps` so booleans read `false`/`true` rather than Python's `False`/`True`. **Deliberately not a CI check** — a service configuration change should not fail somebody's pull request, the same argument that puts `--links` on a weekly schedule. §7.3's discovery-diff proposal remains the scheduled version and is still unbuilt |
| **T2-9** | ✅ **DONE 2026-08-14.**  **Export `errorStatusFrom`**, or record the prohibition | EH-W5's residue (§1.2) | So the next local handler inherits the clamp instead of re-deriving the bug |
| **T2-10** | ✅ **DONE 2026-08-14.**  **The stale line-number sets** | 8693-W3 ⊃ CUR-3b-W6 ⊃ CUR-3c-W10, CUR-3b-W4, CUR-3b-W7, CUR-3c-W12, CUR-3a-W2 | **The advice in the note was the finding: three of this bundle's own replacement numbers had gone stale before anyone applied them.** `parseBearerError` `:20-36` → **`:45`**; the RFC 9470 branch `:81-97` → **`case "FORBIDDEN"` at `:142-167`**; `ParSection.tsx` `:43` → **`:41`**. All were right in early August; T1-1's auth gate and the DPoP-nonce relay grew the controller, and the SPA component shifted by two. **Every ref fixed now carries a content anchor** — *"the `tokenCreateRequest` literal"*, *"the `case \"FORBIDDEN\"` branch"*, *"written there; read back at `:34`"* — so the next drift leaves them findable. **CUR-3c-W12 was closed by deleting the numbers, not correcting them**: T2-1 and T2-11 had already invalidated `:391` and `:182-183`, so seven tutorial citations across two findings now name a **section** instead, which is §6.3's option (b) applied to findings. **CUR-3b-W4 was already satisfied, by deletion** — Module 05 carries no `dpop.service.ts:NNN` ref at all now. One ref found that no item named: `STEP-UP-AUTH-TUTORIAL.md`'s appendix cited `:114`, the *validation-error* branch. `03-curriculum-audit.md`'s copies were left alone — they quote the wrong numbers **as** the defect. **Rule: never cite a line number in a file you are actively rewriting** |
| **T2-11** | ✅ **DONE 2026-08-14.**  **The step-up challenge status everywhere**, including the sequence-diagram arrow | 9470-W1 ⊃ CUR-3c-W6 ⊃ CUR-3b-W8, **+ 9470-W7 (new)** | **"Everywhere" was four locations in the plan and turned out to be five.** `STEP-UP-AUTH-TUTORIAL.md`'s Part 1 arrow and both Part 5 transcripts, Module 09a's ❌ entry — and **`docs/DATA-FLOWS.md`, which drew `RS-->>C: 403 Forbidden` and had never been checked**, because F-1 searched the tutorial, 3c searched the nine tutorials and 3b searched the modules, and a top-level architecture document is none of those. **No code changed**, deliberately: the controller's 403 is the AS→RS introspection response and a defensible vendor mapping, so the defect was always the *conflation*. Part 5 now shows both responses side by side with a six-row boundary table, and the client-action table is split — `acr_values`/`max_age` hang off the **401**, while `acr`/`auth_time` are labelled *Response 1 only*. **Rule recorded in 9470-W7:** a defect stated as *a status code on a particular arrow* recurs wherever that arrow is drawn, so grep the shape across `docs/`, not per document |
| **T2-12** | ✅ **DONE 2026-08-14.**  **Correct CUR-3b-W9's acceptance criteria, then ship it** | CUR-3b-W9 + §2.2 result 2 | **The criteria were stale twice, and the correction is more interesting than the item.** They said the row should read **FAIL**; §2.2 had already noted `ES256` is advertised; and **T1-2 then added `PS256`** on 2026-08-12, so the live list is ten algorithms carrying *both* permitted ones. But `idTokenSignAlg` is pinned **per client** — `ES256` on three clients, **`HS256` on `1523514379`**, which is the client the labs run on. So the row is **PARTIAL**, and the reason it is partial is the finding: **a conformance report written from discovery metadata alone would have scored this PASS.** Module 10's README gains the §5.3.2.1 row plus a four-client table; `lab.md`'s Exercise 7 summary gains it too, and its instruction list now names **PARTIAL** and **NOT REACHABLE** as distinct verdicts from NOT EVIDENCED — because *not reachable* (no measurement exists on this configuration) and *not evidenced* (a measurement exists, unmade) are different reporting errors. Row 7 lands as **NOT REACHABLE**. **The generalisation worth keeping: ask which principal each `shall` binds.** Half of §5.3.2's bind the AS, half bind a client configuration, and `/.well-known` answers only the first |
| **T2-13** | ✅ **DONE 2026-08-14** — the 60× error was in three files, not one.  **Fix the 60s/60min error** in `AGENTS.md` and Module 05 | FAPI1-W1 = FAPI1A-W3 ⊃ CUR-3b-W10 | A spec-accuracy defect in the repo's most-read reference file |
| **T2-14** | ✅ **DONE 2026-08-14.**  **The five citation and section-number fixes** | CUR-3b-W3, CUR-3b-W12, CUR-3b-W13(=NSSO-W3), CUR-3b-W15, CUR-3b-W16, 9470-W4, 9449-W5, 8252-W1 | **The audit's last two fetches are spent, and both went against the audit.** **CUR-3b-W12:** RFC 9101 §10.1 is *Choice of Algorithms* and carries verbatim the MUST-be-signed sentence Module 05 attributes to it — the Phase 2 entry's §6.2 mapping was the incomplete one; §10.8 *Cross-JWT Confusion* exists, settling claim 9. **8252-W1:** §7.3 *Loopback Interface Redirection* is a **MUST** (*“The authorization server MUST allow any port…”*), where this entry's own `<thinking>` said *“should”* — the verdict stands and is stronger, and the *source gap* note is closed. **A by-product neither item predicted closed CUR-3b-W3**: the precedence rule is **§6.3**, the phrase occurs **exactly once** in RFC 9101, and the repo's quotation carried an extra word (*“the parameters **included** in”*). Module 05's **lab had it right and its README did not**, so the two disagreed for a fortnight and the lab was correct. **9470-W4** also touched a file the item could not have named — `utils/step-up.ts`, created by T1-7 — and both code edits are **comment-only**, so `session.controller.ts`'s plan-mode requirement does not apply. **9449-W5** is the one with a live consequence: §8 gives the **AS 400 `use_dpop_nonce`** and §9 the **RS 401**, and `AGENTS.md` said 401 for both — a client retrying only on 401 never retries at the token endpoint, so the wrong status does not mislabel the error, it stops the nonce dance starting. **CUR-3b-W16** names RFC 9440 and adds what its *Informational* status means: it standardises the header, not any obligation to trust it |
| **T2-15** | ✅ **DONE 2026-08-14** *(tutorial halves with T2-1; the rest here).*  **The wire-format gaps, stated until Tier 3 closes them** | 9126-W6 ✅, CIBA-W5 ✅, ~~8628-W5~~ (closed by DR-11), 9101-W4 ✅, 7592-W3 ✅ | Theme 3's honest interim, in the `modules/04…/lab.md` style. **Module 05 gains a four-row table** separating what runs from what does not — and the pattern in it is the transferable part: **PAR is conformant on the way out and not on the way in**, so reading only the response tells you nothing about whether a conformant client could have reached the endpoint. **9101-W4's second clause was stale**: it said *"object signing symmetric-only until W3"*, but W3 shipped on 2026-08-12, so JAR **by value** now runs asymmetrically (`2176571218`, `requestSignAlg: ES256` + a JWK Set); only **by reference** is still unavailable, and the table notes that JAR's §5.2 `request_uri` is a *different* artefact from PAR's. `AGENTS.md`'s CIBA paragraph now states both §7.1 and §7.3 departures and points at the two siblings. **7592-W3 is the sharpest of the five**: every RFC 7592 *operation* is reachable while **none of its HTTP surface is** — no client configuration endpoint, no `GET`/`PUT`/`DELETE`, no `registration_client_uri`; four `POST` routes taking the registration access token in a JSON body. And it sits beside the RFC 7591 half that *was* fixed, so the inventory now says both: **the body is conformant, the endpoint is not** |
| **T2-16** | ✅ **DONE 2026-08-14.**  **The vendor-feature documentation pass** | HSK-W1, HSK-W2, ATTR-W2, ATTR-W4, PS-W1, PS-W2 | All six, and the *“one edit for all three”* instruction was right: `SPEC-INVENTORY.md` gains **one new section** — *Vendor features — implemented here, defined by no specification* — carrying HSK, parameterized scopes and `attributes` together, and stating why they belong in a specification inventory at all: **the most useful thing to know about each is that there is no RFC to check it against.** `docs/API.md` gains the four HSK endpoints (with `DELETE` in a ⚠️ box making the point that *deleting a handle is not deleting a key*), the `attributes` shape with **two** vendor-assigned keys rather than one (`regex` and `fapi2=sp`, the second being the example `FAPI-TUTORIAL.md` already depends on), and a two-row table naming exactly what *no scope management* blocks. **One deliberate departure from HSK-W2**: no separate `docs/` page — a whole page for four endpoints nothing else consumes would be prose in search of a reader; promote it if HSK ever gains a consumer. **PS-W2 earned a four-row table instead of a sentence**, because the new state is the *inverse* of the other three: they describe a capability that is listed and underdelivers, this one is **not listed and works**. The column that makes it click names what misreading each state costs — for this one, *a capability you never knew you had* — and the punchline is that in a capability matrix it does not look like a green tick, it looks like the feature is **absent** |
| **T2-17** ✅ | **Remaining documentation items — SHIPPED 2026-08-15, 8 commits.** 54 IDs listed, **15 already closed** (8 of them recorded nowhere), 39 done, **1 owed**: JARM-W6, a vendor bug report no commit can discharge. **13 of the 39 were not documentation** despite the row's title — 6 code, 2 tests, 4 Authlete writes, 1 vendor report — and 3 of the code items were in a Security-critical surface, so this row contained the session's only plan-mode change. Full record: `PROGRESS.md`, 2026-08-15. | ~~6749-W1~~ ✅ **shipped 2026-08-13 as code, not documentation** — Authlete does not reject dual-channel client credentials, so the escape clause did not apply; `400 invalid_request` at `/api/token` and `/api/par`. 6749-W2, 7521-W2, 8705-W2, 8705-W3, 8707-W2, 9068-W3, 9068-W4, 9207-W2, 9701-W3, 9701-W5, 9728-W3, 8414-W3, 8414-W4, 9396-W2, 8693-W1, 8693-W2, JOSE-W4, JOSE-W5, CIMD-W4, GM-W3 ⚙️, GM-W5, MDL-W2, MCP-W1, 7591-W2, 7662-W4, 7662-W5, 9700-W3, 9700-W5, 8707-W1, 8707-W3, CUR-3a-W6, CUR-3c-W8, CUR-3c-W9, CUR-3c-W13, CUR-3c-W14, JARM-W2, JARM-W4, JARM-W5, JARM-W6, 7523-W2 (= JOSE-W3), 7523-W3 📋, 7523-W5, 9449-W6, 9449-W7 ⚙️, FAPI1A-W4, FAPI1A-W5, MS-W2, FCL-W3, BCL-W4, BCL-W5, **RPL-W5**, **B1-W5**, **CUR-3b-W14** | Independent; batch by file. **BCL-W5 is unblocked** — T0-2 shipped 2026-08-11. **RPL-W5 ✅ shipped with T0-4 (2026-08-12)** — remove it from this batch. The departure it names changed shape in the process: not *"we match origins, not registered values"* but *"we match registered values, held here because Authlete models none"*. ~~CUR-3b-W14~~ ✅ **closed by T1-6 (2026-08-12)** — all four markers deleted rather than re-dated; remove it from this batch |
...
| ~~**RP-Initiated Logout F-1's second half** — no rate limiter on `GET`/`POST /api/logout`~~ | ✅ **CLOSED 2026-08-20.** `generalLimiter` (60/min, per IP) on both methods, ahead of `csrfProtection` so a refusal costs no token work — the same ordering `session.routes.ts` and `device.routes.ts` use. Not `loginLimiter`: logout is not a credential-guessing surface, and 5/min breaks the one-GET-per-POST pattern the single-use CSRF token forces. Two loop-until-429 tests, placed last in the file because a saturated limiter stays saturated for the window | n/a — this row is closed, not standing |
