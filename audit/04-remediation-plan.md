...
| **T1-19** | **Remaining contained code items**: `normalizeGrantType` total; echoing `default` branches; `attributes` validated; duplicate client-listing `fetch()`; `Cache-Control: no-store` on the logout receiver; `parseBearerError` quoted-string aware; PRM `bearer_methods_supported`; PRM well-known path; 405 on `/api/par`; JAR built from named fields; dev JWT made §2-shaped; `computeFapiMode` total over `fapiModes`; `/api/fapi/status` reports the whole profile | B1-W3 📋, B1-W4 ✅(=7009-W3), ATTR-W1 ✅, BCL-W6 ✅, BCL-W7 ✅, 9470-W6 ✅, 9728-W2 ✅, 9728-W1 ✅, 9126-W3 ✅, 9101-W5 📋, 9068-W2 📋, FAPI1-W2 ✅, FAPI2-W4 ✅ ⊃ 7636-W2 | Independent of each other; batch freely. **FAPI2-W4 completes what FAPI2-W1 started** — the endpoint can then fail honestly on all eight FAPI requirements instead of six. **Batch 1 shipped 2026-08-14** (9728-W1/W2, 9126-W3, 9470-W6, B1-W4, FAPI2-W4); *three of those were one defect — an endpoint answering **200 with HTML** where it meant "no"*. **Batch 2 shipped 2026-08-14** (FAPI1-W2, ATTR-W1, BCL-W6) and BCL-W6 corrected an `AGENTS.md` generalisation about the SDK that would have made its own fix silently deliver to nobody — see the entry. **Three remain, all under plan mode: B1-W3, 9101-W5 and 9068-W2.** 9068-W2 is the correction: this row and `RESUME.md` §0 both grouped it with the safe items, but making the dev JWT §2-shaped threads `client_id`/`scope` through `token.operations.service.ts` **and** `token.management.controller.ts`, both listed under **Token issuance**. The finding's own ordering note flagged only W1 as needing a plan, having judged the *file* (`createLocalJWT.ts`, not listed) rather than the *call chain*. **The trigger is the concern, and the concern travels with the parameter.** |
