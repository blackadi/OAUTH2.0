...
| Native SSO | **Enabled and verified 2026-09-03** ([DR-04](audit/05-decision-records.md#dr-04--native-sso), reversed) — both phases pass `scripts/native-sso-verify.mjs`, **14/14**, including public clients, which is the client type Native SSO exists to serve. Caveat: the specification is an **OpenID 2nd Implementer's Draft**, not Final. Public-client safety depends on a refusal this server enforces and Authlete cannot express — see the tutorial before copying the configuration | [Native SSO Tutorial](docs/NATIVE-SSO-TUTORIAL.md) |
