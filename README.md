...
| Native SSO | **Enabled and verified 2026-09-03** ([DR-04](audit/05-decision-records.md#dr-04--native-sso), reversed) — both phases pass `scripts/native-sso-verify.mjs`, 13/13. Two caveats that are not defects: the specification is still **draft 07**, and `tokenExchangeByConfidentialClientsOnly` refuses Phase 2 to public clients, which is the client type Native SSO exists to serve | [Native SSO Tutorial](docs/NATIVE-SSO-TUTORIAL.md) |
