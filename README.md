...
| Native SSO | **Declined** — `nativeSsoSupported` is `false` by decision ([DR-04](audit/05-decision-records.md#dr-04--native-sso)). Enabling the flag is not sufficient: this server returns HTTP 500 on Phase 1 because it never mints a `device_secret`, and Phase 2 is refused for the public clients Native SSO exists to serve | [Native SSO Tutorial](docs/NATIVE-SSO-TUTORIAL.md) |
