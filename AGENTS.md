...
- **`authleteApi.service.get()` currently throws on this service, and it is an SDK enum gap, not a config error** (diagnosed 2026-08-06). Authlete returns `supportedTokenAuthMethods` including `SPIFFE_JWT`; SDK 1.0.0's `ClientAuthMethod` is a **strict** Zod enum of eight members that does not include it, so one unrecognised value rejects the whole 129-field response. Consequence: `GET /api/fapi/config` and `GET /api/fapi/status` both return **HTTP 200 with an error body and a stack trace** — the only two `service.get()` call sites in the server, both in `fapi.controller.ts`. The general fragility matters more than this member: any client-auth method Authlete adds breaks `service.get()` for every TypeScript SDK caller whose service enables it. Escape routes are to drop `SPIFFE_JWT` from the service (if unused) or wait for an SDK that knows it — **not** a `patch-package` patch. Module 10 Exercise 4 teaches the 200-with-stack-trace as a finding, so fixing the local half retires that exercise; see **Deliberate defects** above before changing it.
- **`fapi.controller.ts:38,40-43` hardcodes `requiredClientAuth`, `parRequired`, `pkceRequired`, `scopeRequired` and `refreshTokenRotation`** in `GET /api/fapi/config`, while `GET /api/fapi/status` reads the real values from the service. This is a plain reporting bug, not teaching material: once `service.get()` works again, `config` will assert values the service may not hold. FAPI 2.0 also permits `tls_client_auth`, so the hardcoded `PRIVATE_KEY_JWT` would misreport an mTLS deployment.
