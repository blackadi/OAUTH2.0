...
- **DPoP nonce flow — and the status code differs by *which* server answers** (corrected 2026-08-14, 9449-W5).
  Nonces are OPTIONAL, controlled by `dpopNonceRequired`, which is **`false`** on this service — so **none of
  this is exercisable here** and every line below is the specification's rather than an observation.
