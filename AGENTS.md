...
- **E2E tests**: 1 file `tests/e2e/e2e.test.ts` (**101 tests: 99 pass, 2 skipped outside development**) —
  ...
  > `[A214301]`.
- Run with `npm --prefix server run test` — **1130 tests across 77 files**, completes in ~3s. **Do not carry these numbers forward from memory; re-run and read them.** Client: `npm --prefix client run test` — **410 tests across 39 files**, plus `test:coverage` (ratcheted thresholds), `check:theme`, `check:codes`, `check:docs`
