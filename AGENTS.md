...
# Server tests
npm --prefix server run test              # unit + integration (246 tests, 38 files)
npm --prefix server run test:watch        # watch mode
npm --prefix server run test:coverage     # run with coverage report
npm --prefix server run test:unit         # unit tests only (223 tests, 37 files)
...
- **Unit tests**: 37 files across 5 categories (223 tests):
  - `tests/unit/services/` — 20 files (78 tests), each service in isolation with mocked SDK (includes consent-store, device, metrics, par)
