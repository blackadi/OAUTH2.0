...
- **`post_logout_redirect_uri` is compared by parsed origin, never by string prefix** (fixed 2026-08-10; `isAllowedPostLogoutRedirectUri` in `src/services/logout.service.ts`). `startsWith` against `ALLOWED_ORIGINS` was an **open redirect**, verified live in both directions: with `ALLOWED_ORIGINS=http://localhost:3000`, the value `http://localhost:3000.evil.example.com/bye` passed because the allowed origin is a *prefix of the attacker's hostname*, and `http://localhost:3001@evil.example.com/` passed because everything before `@` is *userinfo* and the real host is the attacker's. The second form survived `NODE_ENV=production`. The rule now:

  | Accepted when | Notes |
  |---|---|
  | the value is byte-identical to `LOGOUT_REDIRECT_URI` | full-URI equality, any environment |
  | `new URL(value).origin` exactly equals an `ALLOWED_ORIGINS` entry's origin | entries are normalised through `new URL()`; a malformed entry is dropped rather than widening the allowlist |
...
- **`authleteApi.service.get()` currently throws on this service, and it is an SDK enum gap, not a config error** (diagnosed 2026-08-06). Authlete returns `supportedTokenAuthMethods` including `SPIFFE_JWT`; SDK 1.0.0's `ClientAuthMethod` is a **strict** Zod enum of eight members that does not include it, so one unrecognised value rejects the whole 129-field response. Consequence: `GET /api/fapi/config` and `GET /api/fapi/status` both fail — the only two `service.get()` call sites in the server, both in `fapi.controller.ts`. The general fragility matters more than this member: any client-auth method Authlete adds breaks `service.get()` for every TypeScript SDK caller whose service enables it. Escape routes are to drop `SPIFFE_JWT` from the service (if unused) or wait for an SDK that knows it — **not** a `patch-package` patch.

  **Two separable layers, and only one of them is about `SPIFFE_JWT`** (separated 2026-08-11). The enum gap explains *why the call fails*. It does not explain why the failure used to be served as **HTTP 200 with an error body** — that was `middleware/errorHandler.ts` deriving the HTTP status from the thrown error, and `AuthleteError` subclasses carry the status of the response they were *reading*, which for a 200 body that fails Zod validation is 200. **That half is fixed**: `errorStatusFrom()` now trusts an error-supplied status only inside 400–599, so both endpoints return a clean **500**. See the DPoP-adjacent note below and `tests/unit/routes/fapi.routes.test.ts`.

  So there is a **third escape route** the two above miss, and it was the one with no curriculum cost. Fixing the status inversion does **not** retire Module 10 Exercise 4 — the endpoints are still broken, just honestly so. **Dropping `SPIFFE_JWT` is what would retire it**; see **Deliberate defects** above before doing that.
